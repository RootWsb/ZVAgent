...
class CloudClient(LinkAIClient):
    ...
    def _remove_weixin_credentials():
        ...
        cred_path = expand_path(
            conf().get("weixin_credentials_path", "runtime/credentials/weixin_zvagent_credentials.json")
        )
        try:
    ...
    def on_chat(self, data: dict, send_chunk_fn):
        ...
        # Intercept zv/slash commands before the agent runs
        try:
            from plugins import PluginManager
            mgr = PluginManager()
            instance = mgr.instances.get("ZV_CLI")
            ...
            logger.warning(f"[CloudClient] zv_cli intercept failed: {e}")
...
ZV_SEND_WEB_SUBDIR = "zvagent-send"


def copy_send_file(src_path: str, workspace_root: str) -> str:
    """Copy *src_path* into ``websites/zvagent-send/`` and return its URL.
    ...
    send_dir = os.path.join(ws, "websites", ZV_SEND_WEB_SUBDIR)
    try:
    ...
    try:
        ...
        return ""
    return f"{base}/{ZV_SEND_WEB_SUBDIR}/{dest_name}"
