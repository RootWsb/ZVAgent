...
class TestQianfanConstantsAndRouting(unittest.TestCase):
    ...
    def test_zv_cli_routes_ernie_models_to_qianfan(self):
        from common import const
        import plugins

        old_plugin_path = plugins.instance.current_plugin_path
        zv_cli_was_registered = "ZV_CLI" in plugins.instance.plugins
        old_zv_cli_plugin = plugins.instance.plugins.get("ZV_CLI")
        parent_had_zv_cli = hasattr(plugins, "zv_cli")
        old_parent_zv_cli = getattr(plugins, "zv_cli", None)
        module_names = ("plugins.zv_cli", "plugins.zv_cli.zv_cli")
        old_modules = {
        ...
        plugins.instance.current_plugin_path = os.path.join(
            os.path.dirname(__file__), "..", "plugins", "zv_cli"
        )
        try:
            import plugins.zv_cli.zv_cli
            zv_cli_plugin = plugins.instance.plugins["ZV_CLI"]
        finally:
            plugins.instance.current_plugin_path = old_plugin_path
            if zv_cli_was_registered:
                plugins.instance.plugins["ZV_CLI"] = old_zv_cli_plugin
            else:
                plugins.instance.plugins.pop("ZV_CLI", None)
            for name in module_names:
                if name in old_modules:
                    sys.modules[name] = old_modules[name]
                else:
                    sys.modules.pop(name, None)
            if parent_had_zv_cli:
                plugins.zv_cli = old_parent_zv_cli
            elif hasattr(plugins, "zv_cli"):
                delattr(plugins, "zv_cli")

        self.assertEqual(
            zv_cli_plugin._resolve_bot_type_for_model("ernie-4.5-turbo-128k"),
            const.QIANFAN,
        )
        self.assertEqual(
            zv_cli_plugin._resolve_bot_type_for_model("qianfan"),
            const.QIANFAN,
        )
