...
from common.user_identity import apply_context_identity
from config import conf


class WebRequestCancelled(BaseException):
    """Raised inside the active web request thread when the user cancels it."""
...
def _check_auth():
    ...
    if not _is_password_enabled():
        return True
    return (
        _verify_auth_token(web.cookies().get("zvagent_auth_token", ""))
        or _verify_auth_token(web.cookies().get("cow_auth_token", ""))
    )
...
def _get_upload_dir() -> str:
    ...
    ws_root = expand_path(conf().get("agent_workspace", "runtime/workspace"))
...
class WebChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        self.cancelled_requests = set()  # request_ids cancelled by the web UI
        self.cancelled_request_times = {}  # request_id -> cancellation timestamp
    ...
    def _fail_callback(self, session_id, exception, **kwargs):
        if isinstance(exception, WebRequestCancelled):
            ...
            self.cancelled_request_times.pop(request_id, None)
            logger.info(f"[WebChannel] Cancelled request {request_id} for session {session_id}")
            return
        return super()._fail_callback(session_id, exception, **kwargs)
    ...
    def _success_callback(self, session_id, **kwargs):
        ...
        if request_id in self.cancelled_requests:
            ...
            self.cancelled_requests.discard(request_id)
            self.cancelled_request_times.pop(request_id, None)
            logger.info(f"[WebChannel] Cleaned cancelled request {request_id} for session {session_id}")
            return
        return super()._success_callback(session_id, **kwargs)

    def _cleanup_cancelled_requests(self):
        cutoff = time.time() - 120
        stale_ids = [rid for rid, ts in self.cancelled_request_times.items() if ts < cutoff]
        for rid in stale_ids:
            self.cancelled_request_times.pop(rid, None)
            self.cancelled_requests.discard(rid)
    ...
    def send(self, reply: Reply, context: Context):
        try:
            ...
            if not request_id:
                ...
                return
            if request_id in self.cancelled_requests:
                logger.info(f"[WebChannel] Dropping reply for cancelled request {request_id}")
                return
            ...
            if request_id in self.sse_queues:
                ...
                if reply.type == ReplyType.VOICE:
                    ...
                    file_name = os.path.basename(str(content))
                    text_content = getattr(reply, 'text_content', '')
                    self._persist_audio_attachment(session_id, str(content), file_name)
                    self.sse_queues[request_id].put({
                        "type": "audio",
                        "content": f"/api/file?path={quote(str(content))}",
                        "file_name": file_name,
                    ...
                    logger.debug(f"SSE audio sent for request {request_id}")
                    return
            ...
            if session_id in self.session_queues:
                ...
                if reply.type == ReplyType.VOICE:
                    ...
                    logger.debug(f"Audio response sent to poll queue for session {session_id}, request {request_id}")
                    return
    ...
    def _persist_audio_attachment(self, session_id: str, file_path: str, file_name: str = ""):
        try:
            if not conf().get("conversation_persistence", True):
                return
            from agent.memory import get_conversation_store
            get_conversation_store().append_assistant_audio(
                session_id,
                file_path,
            ...
            logger.warning(f"[WebChannel] Failed to persist audio attachment: {e}")
    ...
    def _make_sse_callback(self, request_id: str):
        ...
        def on_event(event: dict):
            if request_id in self.cancelled_requests:
                raise WebRequestCancelled(f"Web request {request_id} was cancelled")
            ...
            if event_type == "reasoning_update":
                ...
                is_audio = file_type == "audio" or os.path.splitext(file_path)[1].lower() in AUDIO_EXTENSIONS
                q.put({
                    "type": "image" if is_image else ("audio" if is_audio else "file"),
                    "content": web_url,
                    "file_name": file_name,
                })
    ...
    def post_message(self):
        ...
        try:
            ...
            request_id = json_data.get('request_id') or self._generate_request_id()
            if request_id in self.cancelled_requests:
                self.cancelled_requests.discard(request_id)
                self.cancelled_request_times.pop(request_id, None)
                return json.dumps({"status": "cancelled", "request_id": request_id, "stream": use_sse})
            self.request_to_session[request_id] = session_id
            ...
            # Keep session_id for conversation isolation, but use one stable web
            # identity for quota/user management. Otherwise every new web chat
            # session becomes a separate billing user.
            apply_context_identity(context, "web", "console", "", "Web 控制台")
    ...
    def cancel_request(self):
        """Cancel a web chat request and stop sending any remaining stream events."""
        try:
            data = web.data()
            ...
            if not request_id:
                return json.dumps({"status": "error", "message": "Missing request_id"})

            mapped_session = self.request_to_session.get(request_id)
            if session_id and mapped_session and mapped_session != session_id:
                return json.dumps({"status": "error", "message": "Request does not belong to session"})

            self.cancelled_requests.add(request_id)
            self.cancelled_request_times[request_id] = time.time()
            if mapped_session:
                self.cancel_session(mapped_session)
    ...
    def startup(self):
        ...
        urls = (
            ...
            '/api/reports/detail/(.*)', 'ReportDetailHandler',
            '/api/reports/sources/(.*)', 'ReportSourcesHandler',
            '/api/reports/download/(.*)', 'ReportDownloadHandler',
            '/poll', 'PollHandler',
            ...
            '/api/skill-evolution', 'SkillEvolutionHandler',
            ...
            '/api/history/(.*)/(.*)', 'HistoryTurnHandler',
        ...
        try:
            ...
            if e.errno in (48, 98):  # macOS/Linux EADDRINUSE
                logger.error(
                    f"[WebChannel] 端口 {port} 已被占用，可执行 `zv restart` 清理残留进程，"
...
class AuthLoginHandler:
    def POST(self):
        ...
        web.setcookie("zvagent_auth_token", token, expires=_session_expire_seconds(),
...
class AuthLogoutHandler:
    def POST(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        web.setcookie("zvagent_auth_token", "", expires=-1, path="/")
        return json.dumps({"status": "success"})
...
class FileServeHandler:
    ...
    def _allowed_roots():
        ...
        roots = [
            os.path.abspath(_get_workspace_root()),
            os.path.abspath(get_root()),
            os.path.abspath(os.getcwd()),
        ]
        return list(OrderedDict.fromkeys(roots))

    @classmethod
    def _resolve_file_path(cls, requested_path: str) -> str:
        requested_path = (requested_path or "").strip().strip('"').strip("'")
        if not requested_path:
            raise web.notfound()
        from urllib.parse import unquote
        for _ in range(3):
            decoded = unquote(requested_path)
            if decoded == requested_path:
                break
        ...
        search_prefixes = [
            ...
            "visuals",
            "figures",
            "knowledge/research/figures",
            "knowledge/crypto/figures",
            "knowledge/economics/figures",
            "knowledge/ai-tech/figures",
        ]
        ...
        def variants(path: str):
            ...
            if not ext:
                paths.extend(path + suffix for suffix in image_exts)
            return paths

        candidates = []
        if os.path.isabs(requested_path):
            candidates.extend(os.path.normpath(p) for p in variants(requested_path))
        else:
            rel_path = os.path.normpath(requested_path.lstrip("/\\"))
            for root in cls._allowed_roots():
                for rel_variant in variants(rel_path):
                    candidates.append(os.path.normpath(os.path.join(root, rel_variant)))
                if os.sep not in rel_path and "/" not in rel_path:
                    for prefix in search_prefixes:
                        if not prefix:
                            continue
                        for rel_variant in variants(rel_path):
                            candidates.append(os.path.normpath(os.path.join(root, prefix, rel_variant)))
    ...
    def GET(self):
        ...
        try:
            ...
            file_path = self._resolve_file_path(params.path)
...
class ReportDownloadHandler:
    def GET(self, job_id):
        try:
            from urllib.parse import quote
            from agent.reports.store import ReportStore, reports_root

            params = web.input(token="")
            job = ReportStore().get_job(str(job_id or "").strip())
            token_ok = params.token and hmac.compare_digest(str(params.token), str(job.get("token", "")))
            if not job or not (token_ok or _check_auth()):
                raise web.notfound()

            file_path = os.path.normpath(job.get("pdf_path", ""))
            root = os.path.normpath(reports_root())
            if not file_path or os.path.commonpath([os.path.abspath(root), os.path.abspath(file_path)]) != os.path.abspath(root):
                raise web.notfound()
            if not os.path.isfile(file_path):
                raise web.notfound()

            file_name = os.path.basename(file_path) or f"{job_id}.pdf"
            ...
            logger.error(f"[WebChannel] Error serving report: {e}")
            raise web.notfound()


class ReportsHandler:
    @staticmethod
    def _source_meta(job: dict) -> dict:
        meta = {
            "source_count": int(job.get("source_count") or 0),
            "source_status": job.get("source_status", ""),
            "source_label": job.get("source_label", ""),
            "source_notice": job.get("source_notice", ""),
        }
        try:
            from agent.reports.store import reports_root

            raw_path = str(job.get("sources_path", "") or "").strip()
            if not raw_path:
                return ReportsHandler._normalize_source_meta(meta)
            path = os.path.normpath(raw_path)
            root = os.path.normpath(reports_root())
            if not path or os.path.commonpath([os.path.abspath(root), os.path.abspath(path)]) != os.path.abspath(root):
                return ReportsHandler._normalize_source_meta(meta)
            ...
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            meta.update({
                "source_count": int(data.get("source_count") or len(data.get("sources", []) or [])),
                "source_status": data.get("source_status", meta.get("source_status", "")),
                "source_label": data.get("source_label", meta.get("source_label", "")),
                "source_notice": data.get("source_notice", meta.get("source_notice", "")),
            })
        except Exception:
            pass
        return ReportsHandler._normalize_source_meta(meta)
    ...
    @staticmethod
    def _normalize_source_meta(meta: dict) -> dict:
        count = int(meta.get("source_count") or 0)
        meta["source_count"] = count
        if count > 0:
            meta["source_status"] = meta.get("source_status") or "verified"
            meta["source_label"] = meta.get("source_label") or f"{count} 个外部来源"
            return meta
        meta["source_status"] = meta.get("source_status") or "source_limited"
        meta["source_label"] = meta.get("source_label") or "外部来源待补充"
        meta["source_notice"] = meta.get("source_notice") or "未获取到可验证外部来源，报告已标注资料限制。"
        return meta
    ...
    def GET(self):
        ...
        try:
            ...
            params = web.input(page='1', page_size='50', user_id='')
            result = ReportStore().list_jobs(
                page=int(params.page),
                page_size=int(params.page_size),
                user_id=str(params.user_id or ""),
            )
            for item in result.get("items", []):
                item.update(self._source_meta(item))
            return json.dumps({"status": "success", **result}, ensure_ascii=False)
        except Exception as e:
            logger.error(f"[WebChannel] Reports API error: {e}")
            return json.dumps({"status": "error", "message": str(e)})
    ...
    def DELETE(self):
        ...
        try:
            ...
            if not job_id:
                return json.dumps({"status": "error", "message": "job_id required"})
            deleted = ReportStore().delete_job(job_id)
            return json.dumps({"status": "success", "deleted": deleted}, ensure_ascii=False)
        except Exception as e:
            logger.error(f"[WebChannel] Reports DELETE error: {e}")
            return json.dumps({"status": "error", "message": str(e)})


class ReportSourcesHandler:
    def GET(self, job_id):
        _require_auth()
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            ...
            if not raw_path:
                ...
                root = os.path.normpath(reports_root())
                if os.path.commonpath([os.path.abspath(root), os.path.abspath(file_path)]) != os.path.abspath(root):
                    raise web.notfound()
                if not os.path.isfile(file_path):
                    payload = {
                        "job_id": job.get("job_id", ""),
            ...
            logger.error(f"[WebChannel] Report sources API error: {e}")
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)


class ReportDetailHandler:
    @staticmethod
    ...
    def GET(self, job_id):
        ...
        try:
            ...
            store = ReportStore()
            job = store.get_job(str(job_id or "").strip())
            if not job:
                raise web.notfound()

            root = reports_root()
            safe_job = dict(job)
            safe_job.pop("token", None)
            md_path = self._safe_path(job.get("md_path", ""), root)
            sources_path = self._safe_path(job.get("sources_path", ""), root)
            plan_path = self._safe_path(job.get("plan_path", ""), root)

            markdown = self._read_text(md_path)
            sources = self._read_json(sources_path, {"source_count": 0, "sources": []})
            ...
            logger.error(f"[WebChannel] Report detail API error: {e}")
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)
...
class ConfigHandler:
    ...
    EDITABLE_KEYS = {
        ...
        "enable_thinking", "web_password", "event_report_url", "event_report_port",
    }
    ...
    @staticmethod
    def _extract_port_from_url(url: str, default_port: int = 9119) -> int:
        """Return the port from an event report URL."""
        try:
    ...
    def GET(self):
        ...
        try:
            ...
            title = "ZVAgent" if use_agent else "AI Assistant"
            ...
            return json.dumps({
                ...
                "event_report_url": local_config.get("event_report_url", "http://127.0.0.1:9119/api/sidecar/events"),
                "event_report_port": self._extract_port_from_url(
                    local_config.get("event_report_url", "http://127.0.0.1:9119/api/sidecar/events")
                ),
            }, ensure_ascii=False)
        except Exception as e:
        ...
        try:
            ...
            for key, value in updates.items():
                if key not in self.EDITABLE_KEYS:
                    continue
                if key == "event_report_port":
                    port = int(value)
                    if port < 1 or port > 65535:
                        raise ValueError("event_report_port must be between 1 and 65535")
                    key = "event_report_url"
                    value = f"http://localhost:{port}/api/events"
...
class ChannelsHandler:
    ...
    @staticmethod
    def _get_weixin_login_status(channel_name: str = "weixin") -> str:
        try:
            import sys
    ...
    @staticmethod
    def _channel_base_name(channel_name: str) -> str:
        return str(channel_name or "").split(":", 1)[0]

    @staticmethod
    def _channel_instance_id(channel_name: str) -> str:
    ...
    @staticmethod
    def _next_weixin_account_id(accounts: dict, active_channels: set) -> str:
        used = set()
        if isinstance(accounts, dict):
            used.update(str(key) for key in accounts.keys())
        for ch in active_channels:
            if str(ch).startswith("weixin:"):
                used.add(str(ch).split(":", 1)[1])
        idx = 1
        while f"wx{idx}" in used:
            idx += 1
        return f"wx{idx}"

    @classmethod
    def _build_weixin_channel_info(cls, account_id: str, account_cfg: dict, active_channels: set, candidate: bool = False) -> dict:
        ch_name = f"weixin:{account_id}"
        cfg = account_cfg if isinstance(account_cfg, dict) else {}
        fields_out = []
        for key, label, ftype, default in [
            ("token", "Token", "secret", ""),
            ("base_url", "Base URL", "text", ""),
            ("cdn_base_url", "CDN Base URL", "text", ""),
            ("credentials_path", "Credentials Path", "text", f"runtime/credentials/weixin_zvagent_{account_id}_credentials.json"),
        ]:
            raw_val = cfg.get(key, default)
            display_val = cls._mask_secret(str(raw_val)) if ftype == "secret" and raw_val else raw_val
            fields_out.append({
                "key": key,
                "label": label,
                "type": ftype,
        ...
        return {
            "name": ch_name,
            "label": {"zh": f"微信 {account_id}", "en": f"WeChat {account_id}"},
            "icon": cls.CHANNEL_DEFS["weixin"]["icon"],
            "color": cls.CHANNEL_DEFS["weixin"]["color"],
            "active": ch_name in active_channels,
            "fields": fields_out,
            "login_status": cls._get_weixin_login_status(ch_name) if ch_name in active_channels else "unknown",
            "weixin_account": account_id,
            "candidate": bool(candidate),
        }

    @classmethod
    def _apply_channel_config(cls, local_config: dict, channel_name: str, updates: dict) -> dict:
        if cls._channel_base_name(channel_name) != "weixin" or not cls._channel_instance_id(channel_name):
            local_config.update(updates)
            return updates

        account_id = cls._channel_instance_id(channel_name)
        accounts = local_config.get("weixin_accounts", {}) or {}
        if not isinstance(accounts, dict):
            accounts = {}
        account_cfg = accounts.setdefault(account_id, {})
        account_cfg.update(updates)
        if "credentials_path" not in account_cfg:
            account_cfg["credentials_path"] = f"runtime/credentials/weixin_zvagent_{account_id}_credentials.json"
        local_config["weixin_accounts"] = accounts
        return {"weixin_accounts": accounts}

    @classmethod
    def _valid_channel_keys(cls, channel_name: str, ch_def: dict) -> set:
        valid_keys = {f["key"] for f in ch_def["fields"]}
        if cls._channel_base_name(channel_name) == "weixin" and cls._channel_instance_id(channel_name):
            valid_keys.update({"token", "base_url", "cdn_base_url", "credentials_path"})
        return valid_keys

    @classmethod
    def _secret_channel_keys(cls, channel_name: str, ch_def: dict) -> set:
    ...
    def GET(self):
        ...
        try:
            ...
            for ch_name, ch_def in self.CHANNEL_DEFS.items():
                if ch_name == "weixin":
                    # Personal WeChat is managed as explicit multi-account
                    # instances (weixin:wx1, weixin:wx2, ...). Keeping the
                    # legacy bare "weixin" in the UI makes the third account
                    # look different and starts a separate legacy channel.
                    continue
                fields_out = []
                ...
                if ch_name == "weixin" and ch_name in active_channels:
                    ch_info["login_status"] = self._get_weixin_login_status(ch_name)
                channels.append(ch_info)

            accounts = local_config.get("weixin_accounts", {}) or {}
            if isinstance(accounts, dict):
                for account_id, account_cfg in accounts.items():
                    channels.append(self._build_weixin_channel_info(account_id, account_cfg, active_channels))

            next_account_id = self._next_weixin_account_id(accounts, active_channels)
            channels.append(self._build_weixin_channel_info(next_account_id, {}, active_channels, candidate=True))
    ...
    def POST(self):
        ...
        try:
            ...
            channel_base = self._channel_base_name(channel_name)
            if channel_name not in self.CHANNEL_DEFS and channel_base not in self.CHANNEL_DEFS:
    ...
    def _handle_save(self, channel_name: str, updates: dict):
        ch_def = self.CHANNEL_DEFS[self._channel_base_name(channel_name)]
        valid_keys = self._valid_channel_keys(channel_name, ch_def)
        secret_keys = self._secret_channel_keys(channel_name, ch_def)
        ...
        config_updates = self._apply_channel_config(local_config, channel_name, applied)
        ...
        if os.path.exists(config_path):
            ...
            file_cfg = {}
        file_cfg.update(config_updates)
        with open(config_path, "w", encoding="utf-8") as f:
            json.dump(file_cfg, f, indent=4, ensure_ascii=False)
...
class WeixinQrHandler:
    ...
    @classmethod
    def _state_for(cls, account_id: str) -> dict:
        return cls._qr_state.get(account_id or "", {})

    @classmethod
    def _set_state_for(cls, account_id: str, state: dict):
        key = account_id or ""
        if state:
            cls._qr_state[key] = state
        else:
            cls._qr_state.pop(key, None)

    @staticmethod
    def _account_config(account_id: str) -> dict:
        accounts = conf().get("weixin_accounts", {}) or {}
        if account_id and isinstance(accounts, dict) and isinstance(accounts.get(account_id), dict):
            return accounts[account_id]
        return {}

    @classmethod
    def _account_value(cls, account_id: str, local_key: str, legacy_key: str = None, default=None):
        account = cls._account_config(account_id)
        if local_key in account:
            return account[local_key]
        if legacy_key and legacy_key in account:
            return account[legacy_key]
        return conf().get(legacy_key or local_key, default)

    @staticmethod
    def _default_credentials_path(account_id: str) -> str:
        if account_id:
            return f"runtime/credentials/weixin_zvagent_{account_id}_credentials.json"
        return "runtime/credentials/weixin_zvagent_credentials.json"
    ...
    @classmethod
    def _persist_login_config(cls, account_id: str, bot_token: str, base_url: str, cred_path: str):
        ...
        try:
            ...
            if account_id:
                accounts = file_cfg.get("weixin_accounts", {}) or {}
                if not isinstance(accounts, dict):
                    accounts = {}
                account_cfg = accounts.setdefault(account_id, {})
                account_cfg["token"] = bot_token
                account_cfg["base_url"] = base_url
                account_cfg["credentials_path"] = cred_path
                file_cfg["weixin_accounts"] = accounts
            else:
                file_cfg["weixin_token"] = bot_token
                file_cfg["weixin_base_url"] = base_url
                file_cfg["weixin_credentials_path"] = cred_path

            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(file_cfg, f, indent=4, ensure_ascii=False)
        except Exception as e:
            logger.warning(f"[WebChannel] Failed to persist WeChat login config: {e}")
    ...
    def _qr_to_data_uri(data: str) -> str:
        ...
        try:
            ...
            logger.warning("[WebChannel] qrcode package is not installed; cannot render Weixin QR image")
            return ""
        except Exception as e:
            logger.warning(f"[WebChannel] Failed to render Weixin QR image: {e}")
            return ""

    @staticmethod
    def _qr_dependency_error() -> str:
        return (
            "二维码图片生成失败：当前 Python 环境缺少 qrcode/Pillow 依赖。"
            "请运行 python -m pip install qrcode Pillow 后重启 ZVAgent。"
        )

    @staticmethod
    def _get_running_channel(account_id: str = ""):
        try:
            import sys
            app_module = sys.modules.get('__main__') or sys.modules.get('app')
            mgr = getattr(app_module, '_channel_mgr', None) if app_module else None
            if mgr:
                return mgr.get_channel(f"weixin:{account_id}" if account_id else "weixin")
        except Exception:
            pass
        return None
    ...
    def GET(self):
        ...
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            params = web.input(account="")
            account_id = self._safe_account_id(params.get("account", ""))
            force_new = str(params.get("force", "")).lower() in ("1", "true", "yes", "new")
            running_ch = self._get_running_channel(account_id) if not force_new else None
            if running_ch and hasattr(running_ch, '_current_qr_url') and running_ch._current_qr_url:
                ...
                if not qr_image:
                    return json.dumps({
                        "status": "error",
                        "message": self._qr_dependency_error(),
            ...
            base_url = self._account_value(account_id, "base_url", "weixin_base_url", DEFAULT_BASE_URL)
            ...
            WeixinQrHandler._set_state_for(account_id, {
                "qrcode": qrcode,
                "qrcode_url": qrcode_url,
                "base_url": base_url,
            })
            return json.dumps({"status": "success", "qrcode_url": qrcode_url, "qr_image": qr_image, "account": account_id}, ensure_ascii=False)
        except Exception as e:
    ...
    def POST(self):
        ...
        try:
            ...
            account_id = self._safe_account_id(body.get("account", ""))

            if action == "poll":
                return self._poll_status(account_id)
            elif action == "refresh":
                from channel.weixin.weixin_api import WeixinApi, DEFAULT_BASE_URL
                base_url = self._account_value(account_id, "base_url", "weixin_base_url", DEFAULT_BASE_URL)
                api = WeixinApi(base_url=base_url)
                ...
                self._set_state_for(account_id, {
                    "qrcode": qrcode,
                    "qrcode_url": qrcode_url,
                    "base_url": base_url,
                })
    ...
    def _poll_status(self, account_id: str = ""):
        state = WeixinQrHandler._state_for(account_id)
        ...
        if qr_status == "confirmed":
            ...
            from common.utils import expand_path
            cred_path = expand_path(
                self._account_value(
                    account_id,
                    "credentials_path",
                    "weixin_credentials_path",
                    self._default_credentials_path(account_id),
                )
            )
            ...
            if account_id:
                ...
                account_cfg = accounts.setdefault(account_id, {})
                account_cfg["token"] = bot_token
                account_cfg["base_url"] = result_base_url
                account_cfg["credentials_path"] = cred_path
                conf()["weixin_accounts"] = accounts
            else:
                conf()["weixin_token"] = bot_token
                conf()["weixin_base_url"] = result_base_url
                conf()["weixin_credentials_path"] = cred_path

            self._persist_login_config(account_id, bot_token, result_base_url, cred_path)
        ...
        if qr_status == "expired":
            ...
            WeixinQrHandler._set_state_for(account_id, state)
            return json.dumps({
                "status": "success",
                "qr_status": "expired",
                "qrcode_url": new_qrcode_url,
...
def _get_workspace_root():
    ...
    from common.utils import expand_path
    return expand_path(conf().get("agent_workspace", "runtime/workspace"))
...
class SkillEvolutionHandler:
    @staticmethod
    def _list_skills_for_evolution():
        """Return installed skills with a disk/config fallback for the UI."""
        skills = []
        try:
            from agent.skills.service import SkillService
            from agent.skills.manager import SkillManager
            ...
            logger.warning(f"[WebChannel] Skill evolution query fallback: {e}")

        by_name = {s.get("name"): s for s in skills if s.get("name")}

        try:
            from cli.utils import load_skills_config, get_builtin_skills_dir
            ...
            workspace_root = _get_workspace_root()
            for base, source in (
                (os.path.join(workspace_root, "skills"), "custom"),
            ...
            logger.warning(f"[WebChannel] Skill evolution disk fallback failed: {e}")
    ...
    def GET(self):
        ...
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            params = web.input(action='overview', name='')
            from cli.commands import skill_evolution as evo

            action = params.action
            if action == "overview":
                skills = self._list_skills_for_evolution()
                proposals = evo.list_proposals()
                scores = evo.score_skills([s.get("name", "") for s in skills if s.get("name")])
                return json.dumps({
                    "status": "success",
                    "skills": skills,
                    "proposals": proposals,
            ...
            if action == "suggest":
                ...
                suggestions = evo.collect_skill_suggestions(name)
                return json.dumps({
                    "status": "success",
                    "skill": name,
                    "suggestions": suggestions,
                    "text": evo.format_suggestions(name),
                }, ensure_ascii=False)
            ...
            if action == "proposals":
                ...
                return json.dumps({
                    "status": "success",
                    "proposals": evo.list_proposals(name),
                }, ensure_ascii=False)

            if action == "diff":
                proposal_id = getattr(params, "proposal_id", "").strip()
                if not proposal_id:
                    return json.dumps({"status": "error", "message": "proposal_id required"})
                diff = evo.get_proposal_diff(proposal_id)
                diff["status"] = "success"
            ...
            if action == "replay":
                ...
                if not proposal_id:
                    return json.dumps({"status": "error", "message": "proposal_id required"})
                result = evo.replay_proposal(proposal_id)
                result["status"] = "success"
                return json.dumps(result, ensure_ascii=False)

            if action == "events":
                limit = int(getattr(params, "limit", 100) or 100)
                offset = int(getattr(params, "offset", 0) or 0)
                start_at = getattr(params, "start_at", None) or None
                end_at = getattr(params, "end_at", None) or None
                start_at = int(start_at) if start_at is not None else None
                end_at = int(end_at) if end_at is not None else None
                result = evo.query_events(limit=limit, offset=offset, start_at=start_at, end_at=end_at)
                return json.dumps({
                    "status": "success",
                    **result,
                }, ensure_ascii=False)
            ...
            if action == "backups":
                ...
                return json.dumps({
                    "status": "success",
                    "backups": evo.list_backups(name),
                }, ensure_ascii=False)

            return json.dumps({"status": "error", "message": f"unknown action: {action}"})

        except Exception as e:
            logger.error(f"[WebChannel] Skill evolution GET error: {e}")
            return json.dumps({"status": "error", "message": str(e)})
    ...
    def POST(self):
        ...
        try:
            ...
            if action == "draft":
                ...
                if not name:
                    return json.dumps({"status": "error", "message": "name required"})
                result = evo.create_draft(name)
                return json.dumps({
                    "status": "success",
                    "proposal_id": result.proposal_id,
                    "proposal_dir": result.proposal_dir,
                    "editable_skill_dir": result.editable_skill_dir,
            ...
            if action == "auto_learn":
                ...
                if not name:
                    return json.dumps({"status": "error", "message": "name required"})
                result = evo.create_auto_learn_proposal(name)
                return json.dumps({
                    "status": "success",
                    "proposal_id": result.proposal_id,
                    "proposal_dir": result.proposal_dir,
            ...
            if action == "auto_queue":
                skills = self._list_skills_for_evolution()
                max_items = int(body.get("max_items", 3) or 3)
                result = evo.create_auto_learn_queue(
            ...
            if action == "ai_rewrite":
                ...
                if not proposal_id:
                    return json.dumps({"status": "error", "message": "proposal_id required"})
                result = evo.ai_rewrite_proposal(proposal_id)
                result["status"] = "success"
                return json.dumps(result, ensure_ascii=False)

            if action == "auto_apply_safe":
                max_items = int(body.get("max_items", 3) or 3)
                result = evo.auto_apply_safe(max_items=max_items)
                result["status"] = "success"
                return json.dumps(result, ensure_ascii=False)

            if action == "run_cycle":
                skills = self._list_skills_for_evolution()
                max_queue = int(body.get("max_queue", 3) or 3)
                max_apply = int(body.get("max_apply", 3) or 3)
                enable_ai_rewrite = bool(body.get("enable_ai_rewrite", True))
            ...
            if action == "validate":
                ...
                if not proposal_id:
                    return json.dumps({"status": "error", "message": "proposal_id required"})
                proposal_dir = evo._find_proposal(proposal_id)
                result = evo.validate_skill_dir(os.path.join(proposal_dir, "skill"))
                meta = evo._load_proposal_metadata(proposal_dir)
                evo.record_event(
                    "validate_checked",
            ...
            if action == "apply":
                ...
                if not proposal_id:
                    return json.dumps({"status": "error", "message": "proposal_id required"})
                result = evo.apply_proposal(proposal_id)
                return json.dumps({
                    "status": "success",
            ...
            if action == "rollback":
                name = body.get("name", "").strip()
                backup_id = body.get("backup_id", "").strip() or None
                if not name:
                    return json.dumps({"status": "error", "message": "name required"})
                result = evo.rollback_skill(name, backup_id=backup_id)
                return json.dumps({
                    "status": "success",
                    "skill_name": result.skill_name,
                    "backup_id": result.backup_id,
            ...
            logger.error(f"[WebChannel] Skill evolution POST error: {e}")
            return json.dumps({"status": "error", "message": str(e)})
...
class QuotaHandler:
    def GET(self):
        ...
        try:
            ...
            params = web.input(action='overview', user_id='', limit='200')
            manager = get_quota_manager()
            action = params.action

            if action == "overview":
                manager.consolidate_web_session_users()
                users = manager.list_users(limit=int(params.limit or 200))
                return json.dumps({
                    "status": "success",
                    "enabled": bool(conf().get("quota_enabled", False)),
                    "default_tokens": int(conf().get("quota_default_tokens", 0) or 0),
                    "min_request_tokens": int(conf().get("quota_min_request_tokens", 1000) or 1000),
                    "stats": manager.stats(),
                    "users": users,
                    "redeem_codes": manager.list_redeem_codes(limit=50),
                }, ensure_ascii=False)
            ...
            if action == "detail":
                ...
                user = manager.get_user(user_id) or manager.ensure_user(user_id)
                logs = manager.usage_logs(user_id=user_id, limit=100)
                skill_logs = manager.skill_usage_logs(user_id=user_id, limit=100)
                return json.dumps({"status": "success", "user": user, "logs": logs, "skill_logs": skill_logs}, ensure_ascii=False)

            if action == "skill_logs":
                user_id = params.user_id.strip()
                logs = manager.skill_usage_logs(user_id=user_id, limit=int(params.limit or 100))
                return json.dumps({"status": "success", "logs": logs}, ensure_ascii=False)

            return json.dumps({"status": "error", "message": f"unknown action: {action}"})
        except Exception as e:
            logger.error(f"[WebChannel] Quota GET error: {e}")
            return json.dumps({"status": "error", "message": str(e)})
    ...
    def POST(self):
        ...
        try:
            ...
            if action == "set_config":
                updates = {}
                if "enabled" in body:
                    conf()["quota_enabled"] = bool(body.get("enabled"))
                    updates["quota_enabled"] = bool(body.get("enabled"))
                if "default_tokens" in body:
                    value = max(0, int(body.get("default_tokens") or 0))
                    conf()["quota_default_tokens"] = value
                    updates["quota_default_tokens"] = value
                if "min_request_tokens" in body:
                    value = max(1, int(body.get("min_request_tokens") or 1))
                    conf()["quota_min_request_tokens"] = value
                    updates["quota_min_request_tokens"] = value
                if updates:
                    ...
                    with open(config_path, "w", encoding="utf-8") as f:
                        json.dump(file_cfg, f, indent=4, ensure_ascii=False)
                return json.dumps({"status": "success", "applied": updates}, ensure_ascii=False)

            if action == "generate_redeem":
                codes = manager.generate_redeem_codes(
                    token_amount=int(body.get("token_amount") or body.get("delta_tokens") or 0),
                    count=int(body.get("count") or 1),
                    created_by=str(body.get("created_by", "web-admin") or "web-admin"),
                    note=str(body.get("note", "") or ""),
                )
                return json.dumps({"status": "success", "codes": codes}, ensure_ascii=False)

            if action == "redeem_code":
                if not user_id:
                    return json.dumps({"status": "error", "message": "user_id required"})
                result = manager.redeem_code(
            ...
            if action == "set_quota":
                user = manager.set_quota(
                    user_id,
                    int(body.get("quota_tokens") or 0),
                    channel_type=str(body.get("channel_type", "") or ""),
                    display_name=str(body.get("display_name", "") or ""),
                )
                return json.dumps({"status": "success", "user": user}, ensure_ascii=False)

            if action == "add_quota":
                user = manager.add_quota(
                    user_id,
                    int(body.get("delta_tokens") or 0),
                    channel_type=str(body.get("channel_type", "") or ""),
                    display_name=str(body.get("display_name", "") or ""),
                )
                return json.dumps({"status": "success", "user": user}, ensure_ascii=False)
            ...
            if action == "set_status":
                ...
                if status not in (ACTIVE, DISABLED):
                    return json.dumps({"status": "error", "message": "status must be active or disabled"})
                user = manager.set_status(user_id, status)
                return json.dumps({"status": "success", "user": user}, ensure_ascii=False)

            if action == "set_display_name":
                user = manager.set_display_name(
                    user_id,
            ...
            logger.error(f"[WebChannel] Quota POST error: {e}")
            return json.dumps({"status": "error", "message": str(e)})
...
class SessionDetailHandler:
    def DELETE(self, session_id: str):
        ...
        try:
            ...
            from agent.memory import get_conversation_store
            store = get_conversation_store()
            cleanup_result = store.clear_session(session_id)
            ...
            logger.info(
                f"[WebChannel] Session deleted: {session_id}, "
                f"messages={cleanup_result.get('deleted_messages')}, "
                f"files={len(cleanup_result.get('deleted_files', []))}"
            )
            return json.dumps({"status": "success", **cleanup_result}, ensure_ascii=False)
        except Exception as e:
...
class HistoryTurnHandler:
    def DELETE(self, session_id: str, user_seq: str):
        _require_auth()
        web.header('Content-Type', 'application/json; charset=utf-8')
        try:
            if not session_id:
            ...
            from agent.memory import get_conversation_store
            result = get_conversation_store().delete_visible_turn(session_id, seq)
            ...
            try:
                ...
                if session_id in ab.agents:
                    del ab.agents[session_id]
                    logger.info(f"[WebChannel] Removed agent instance after turn delete: {session_id}")
            except Exception:
                pass

            logger.info(
                f"[WebChannel] Deleted turn: session={session_id}, seq={seq}, "
                f"messages={result.get('deleted_messages')}, files={len(result.get('deleted_files', []))}"
            )
            return json.dumps({"status": "success", **result}, ensure_ascii=False)
        except ValueError as e:
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)
        except Exception as e:
            logger.error(f"[WebChannel] History turn delete error: {e}")
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)
...
class VersionHandler:
    def GET(self):
        web.header('Content-Type', 'application/json; charset=utf-8')
        return json.dumps({"version": "1.0", "release_url": ""})
