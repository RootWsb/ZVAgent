...
def _get_tmp_dir(account_id: str = "") -> str:
    ws_root = expand_path(conf().get("agent_workspace", "runtime/workspace"))
    tmp_dir = os.path.join(ws_root, "tmp", "weixin", _safe_id(account_id)) if account_id else os.path.join(ws_root, "tmp")
...
class WeixinMessage(ChatMessage):
    ...
    def __init__(self, msg: dict, cdn_base_url: str = CDN_BASE_URL, account_id: str = ""):
        super().__init__(msg)
    ...
    def _setup_media(self, item: dict, media_type: int, cdn_base_url: str):
        ...
        if media_type == ITEM_IMAGE:
            ...
            save_path = os.path.join(_get_tmp_dir(self.account_id), f"wx_{self.msg_id}.mp4")
            ...
            save_path = os.path.join(_get_tmp_dir(self.account_id), file_name)
            self.content = save_path
            ...
            save_path = os.path.join(_get_tmp_dir(self.account_id), f"wx_{self.msg_id}.silk")
            self.content = save_path
    ...
    def _download_media(self, item: dict, media_type: int, cdn_base_url: str) -> str:
        ...
        if media_type == ITEM_FILE:
            ...
            if original_name:
                save_path = os.path.join(_get_tmp_dir(self.account_id), original_name)
            else:
                save_path = os.path.join(_get_tmp_dir(self.account_id), f"wx_{self.msg_id}.bin")
        else:
            ext_map = {ITEM_IMAGE: ".jpg", ITEM_VIDEO: ".mp4", ITEM_VOICE: ".silk"}
            ext = ext_map.get(media_type, "")
            save_path = os.path.join(_get_tmp_dir(self.account_id), f"wx_{self.msg_id}{ext}")
