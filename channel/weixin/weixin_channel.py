...
from common.user_identity import apply_context_identity, build_user_key
    ...
    try:
        if os.path.exists(cred_path):
            with open(cred_path, "r") as f:
                return json.load(f)
        legacy_path = cred_path.replace("weixin_zvagent", "weixin_cow")
        if legacy_path != cred_path and os.path.exists(legacy_path):
...
class WeixinChannel(ChatChannel):
    ...
    @classmethod
    def clear_instance(cls, instance_id: str = ""):
        key = instance_id or ""
        with cls._instances_lock:
    ...
    def __init__(self, instance_id: str = ""):
        ...
        self.account_config = self._load_account_config(self.instance_id)
        self.api = None
        self._stop_event = threading.Event()
        self._poll_thread = None
        self._context_tokens = {}  # user_id -> context_token
        self._received_msgs = ExpiredDict(60 * 60 * 7.1)
        self._recent_inbound_fingerprints = ExpiredDict(30)
    ...
    @staticmethod
    def _load_account_config(instance_id: str) -> dict:
        accounts = conf().get("weixin_accounts", {}) or {}
        if instance_id and isinstance(accounts, dict) and isinstance(accounts.get(instance_id), dict):
            return accounts[instance_id]
        return {}

    def _cfg(self, local_key: str, legacy_key: str = None, default=None):
        if local_key in self.account_config:
            return self.account_config[local_key]
        if legacy_key and legacy_key in self.account_config:
            return self.account_config[legacy_key]
        return conf().get(legacy_key or local_key, default)

    def _default_credentials_path(self) -> str:
        if self.instance_id:
            return f"runtime/credentials/weixin_zvagent_{self._safe_id(self.instance_id)}_credentials.json"
        return "runtime/credentials/weixin_zvagent_credentials.json"

    def _session_key(self, user_id: str) -> str:
        return build_user_key("weixin", user_id, self.instance_id)
    ...
    def startup(self):
        ...
        base_url = self._cfg("base_url", "weixin_base_url", DEFAULT_BASE_URL)
        cdn_base_url = self._cfg("cdn_base_url", "weixin_cdn_base_url", CDN_BASE_URL)
        token = self._cfg("token", "weixin_token", "")

        self._credentials_path = expand_path(
            self._cfg("credentials_path", "weixin_credentials_path", self._default_credentials_path())
        )
    ...
    def _process_message(self, raw_msg: dict):
        ...
        msg_id = str(raw_msg.get("message_id", raw_msg.get("seq", ""))).strip()
        msg_key = f"{self.instance_id}:{msg_id}" if msg_id else ""
        if msg_key and self._received_msgs.get(msg_key):
            logger.info(f"[Weixin] Duplicate message_id ignored: {msg_id}")
            return
        if msg_key:
            self._received_msgs[msg_key] = True
        ...
        cdn_base_url = self.api.cdn_base_url if self.api else CDN_BASE_URL
        try:
            wx_msg = WeixinMessage(raw_msg, cdn_base_url=cdn_base_url, account_id=self.instance_id)
        except Exception as e:
            logger.error(f"[Weixin] Failed to parse WeixinMessage: {e}", exc_info=True)
            return

        fingerprint = self._message_fingerprint(raw_msg, wx_msg)
        if self._recent_inbound_fingerprints.get(fingerprint):
            logger.info(
                f"[Weixin] Duplicate message fingerprint ignored: "
                f"from={from_user} msg_id={msg_id or '-'} content={str(wx_msg.content)[:50]}"
            )
            return
        self._recent_inbound_fingerprints[fingerprint] = True
        ...
        session_id = self._session_key(from_user)
        ...
        if context:
            if context.type == ContextType.TEXT:
                try:
                    from agent.reports.service import handle_report_command
                    if handle_report_command(context, self):
                        return
                except Exception as e:
                    logger.error(f"[Weixin] Report command handling failed: {e}", exc_info=True)
            self.produce(context)

    # ── _compose_context ───────────────────────────────────────────────

    def _message_fingerprint(self, raw_msg: dict, wx_msg: WeixinMessage) -> str:
        """Build a short-lived idempotency key for Weixin redeliveries."""
        content = str(getattr(wx_msg, "content", "") or "").strip()
        content = re.sub(r"\s+", " ", content)
        parts = [
            self.instance_id or "",
            str(raw_msg.get("from_user_id", "")),
            str(raw_msg.get("to_user_id", "")),
            str(getattr(wx_msg, "ctype", "")),
            content,
        ]
        raw = "\x1f".join(parts)
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        cmsg = context["msg"]
        context["session_id"] = self._session_key(cmsg.from_user_id)
        context["weixin_account"] = self.instance_id
        context["receiver"] = cmsg.other_user_id
        apply_context_identity(
            context,
            "weixin",
            cmsg.from_user_id,
            self.instance_id,
            getattr(cmsg, "from_user_nickname", "") or cmsg.from_user_id,
        )
    ...
    def _resolve_media_path(path_or_url: str) -> str:
        ...
        if local_path.startswith(("http://", "https://")):
            try:
                ...
                tmp_dir = os.path.join(ws_root, "tmp", "weixin")
                os.makedirs(tmp_dir, exist_ok=True)
                tmp_path = os.path.join(tmp_dir, f"wx_media_{uuid.uuid4().hex[:8]}{ext}")
