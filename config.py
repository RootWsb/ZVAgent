...
available_setting = {
    ...
    "weixin_credentials_path": "runtime/credentials/weixin_zvagent_credentials.json",  # credentials file path
    # Multiple personal WeChat accounts. Use channel_type like:
    # ["web", "weixin:wx1", "weixin:wx2"]
    # Each account has isolated credentials and user sessions.
    "weixin_accounts": {},
    # chatgpt指令自定义触发词
    "clear_memory_commands": ["#清除记忆"],  # 重置会话指令，必须以#开头
    # channel配置
    "channel_type": "",  # 通道类型，支持多渠道同时运行。单个: "weixin"，多个: "weixin, qq" 或 ["weixin", "qq"]。可选值: web,weixin,qq
    "python_executable": "",  # 启动后台服务时使用的 Python 解释器路径；留空则使用当前解释器
    "web_console": True,  # 是否自动启动Web控制台（默认启动）。设为False可禁用
    "web_auto_open_browser": False,  # 启动 Web 控制台时是否自动打开本机浏览器
    "debug": False,  # 是否开启debug模式，开启后会打印更多日志
    "appdata_dir": "runtime/appdata",  # 数据目录
    # 插件配置
    ...
    "crypto_api_key": "",  # CoinGecko API key for crypto news & analysis
    "web_port": 9899,
    "web_password": "",  # Web console password; empty means no authentication required
    "web_session_expire_days": 30,  # Auth session expiry in days
    "report_public_base_url": "",  # Public base URL for report download links.
    "report_duplicate_window_seconds": 120,  # Prevent duplicate report jobs from message retries
    "agent": True,  # 是否开启Agent模式
    "agent_workspace": "runtime/workspace",  # agent工作空间路径，用于存储skills、memory等
    "agent_max_context_tokens": 50000,  # Agent模式下最大上下文tokens
    "agent_max_context_turns": 20,  # Agent模式下最大上下文记忆轮次
    "agent_max_steps": 20,  # Agent模式下单次运行最大决策步数
    "enable_thinking": False,  # Enable deep-thinking mode for thinking-capable models
    "skill_prompt_max_skills": 8,  # Max skills injected into each Agent prompt
    "skill_prompt_description_chars": 180,  # Max chars per skill description in prompt
    "skill_prompt_always_include": ["task-budget-router", "source-registry", "quality-guardrails"],
    "skill_prompt_include_unavailable": False,
    "skill_progressive_loading_enabled": True,  # Hermes-style skill loading: full Level 1 catalog + top-K Level 2 summaries + on-demand SKILL.md reads
    "skill_level1_max_skills": 0,  # 0 means include all enabled skills in the lightweight Level 1 catalog
    "skill_summary_enabled": True,  # Load validated Level 2 summaries for top skill candidates when available
    "skill_summary_max_candidates": 3,  # Max candidate summaries added after Level 1 routing
    "browser_headless": True,  # Agent 浏览器工具默认后台运行，不弹出 Chromium 窗口
    "browser_idle_timeout": 120,  # 浏览器空闲多少秒后自动释放
    "restricted_user_channels": ["weixin", "wecom_bot", "wechatcom_app", "wechatmp", "wechatmp_service"],  # 外部微信类通道默认降权
    "restricted_user_blocked_tools": ["write", "edit", "bash", "env_config", "scheduler"],  # 降权通道不可使用的高危工具
    "restricted_user_allow_mcp_tools": False,  # MCP 工具能力未知，外部用户默认不可用
    "knowledge": True,  # 是否开启知识库功能
    "event_report_enabled": True,
    "event_report_url": "http://127.0.0.1:9119/api/sidecar/events",
    "event_report_user_id": "your-name",
    "event_report_source": "your-personal-assistant",
    # Token quota / commercial MVP. Disabled by default so existing local use
    # will not be interrupted until the owner explicitly enables it.
    "quota_enabled": False,
    "quota_default_tokens": 0,
    "quota_min_request_tokens": 1000,
    "quota_attachment_tokens": 300,
...
def load_config():
    ...
    if config.get("agent", False):
        workspace = config.get("agent_workspace", "runtime/workspace")
...
def get_appdata_dir():
    configured = conf().get("appdata_dir", "runtime/appdata")
    if configured:
        data_path = os.path.abspath(os.path.join(get_root(), configured))
    else:
        data_path = get_runtime_path("appdata")
