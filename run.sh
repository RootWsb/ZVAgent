...
if [ -z "$BASH_VERSION" ]; then
    echo -e "${RED}鉂?Please run this script with Bash.${NC}"
    exit 1
fi
...
check_and_install_tool() {
    ...
    if ! command -v "$tool_name" &> /dev/null; then
        echo -e "${YELLOW}鈿欙笍  $tool_name not found, installing...${NC}"
        if command -v yum &> /dev/null; then
            ...
            echo -e "${RED}鉂?Unsupported package manager. Please install $tool_name manually.${NC}"
            return 1
        fi

        if ! command -v "$tool_name" &> /dev/null; then
            echo -e "${RED}鉂?Failed to install $tool_name.${NC}"
            return 1
        else
            echo -e "${GREEN}鉁?$tool_name installed successfully.${NC}"
            return 0
        fi
    else
        echo -e "${GREEN}鉁?$tool_name is already installed.${NC}"
        return 0
    fi
}
...
detect_python_command() {
    ...
    if [ -z "$PYTHON_CMD" ]; then
        ...
        if [ -n "$FOUND_NEWER_VERSION" ]; then
            echo -e "${RED}鉂?Found Python $FOUND_NEWER_VERSION, but this project requires Python 3.7-3.12${NC}"
            ...
            echo -e "${RED}鉂?No suitable Python found. Please install Python 3.7-3.12${NC}"
        fi
        exit 1
    fi
    ...
    echo -e "${GREEN}鉁?Found Python: $PYTHON_CMD (version $PYTHON_VERSION)${NC}"
}
...
check_python_version() {
    ...
    if ! $PYTHON_CMD -m pip --version &> /dev/null; then
        echo -e "${RED}鉂?pip not found for $PYTHON_CMD. Please install pip.${NC}"
        exit 1
    fi
    
    echo -e "${GREEN}鉁?pip is available for $PYTHON_CMD${NC}"
}

# Clone project
clone_project() {
    echo -e "${GREEN}馃攳 Cloning ZVAgent project...${NC}"

    if [ -d "ZVAgent" ]; then
        echo -e "${YELLOW}鈿狅笍  Directory 'ZVAgent' already exists.${NC}"
        read -p "Choose action: overwrite(o), backup(b), or quit(q)? [press Enter for default: b]: " choice
        choice=${choice:-b}
        case "$choice" in
            o|O)
                echo -e "${YELLOW}馃棏锔? Overwriting 'ZVAgent' directory...${NC}"
                rm -rf ZVAgent
                ;;
            b|B)
                backup_dir="ZVAgent_backup_$(date +%s)"
                echo -e "${YELLOW}馃攢 Backing up to '$backup_dir'...${NC}"
                mv ZVAgent "$backup_dir"
                ;;
            q|Q)
                echo -e "${RED}鉂?Installation cancelled.${NC}"
                exit 1
                ;;
            *)
                echo -e "${RED}鉂?Invalid choice. Exiting.${NC}"
                exit 1
                ;;
        esac
    fi
    ...
    if ! command -v git &> /dev/null; then
        echo -e "${YELLOW}鈿狅笍  Git not available. Trying wget/curl...${NC}"
        local zip_url="https://gitee.com/zhayujie/ZVAgent/repository/archive/master.zip"
        if command -v wget &> /dev/null; then
            wget "$zip_url" -O ZVAgent.zip
        elif command -v curl &> /dev/null; then
            curl -L "$zip_url" -o ZVAgent.zip
        else
            echo -e "${RED}鉂?Cannot download project. Please install Git, wget, or curl.${NC}"
            exit 1
        fi
        unzip ZVAgent.zip
        mv ZVAgent-master ZVAgent
        rm ZVAgent.zip
    else
        ...
        if [ -n "$_http_proxy" ] && ! curl -s --connect-timeout 3 --max-time 5 --proxy "$_http_proxy" https://github.com > /dev/null 2>&1; then
            echo -e "${YELLOW}鈿狅笍  Invalid git proxy detected: $_http_proxy, temporarily disabling...${NC}"
        ...
        if curl -sI --connect-timeout 5 --max-time 10 https://github.com > /dev/null 2>&1; then
            echo -e "${YELLOW}馃寪 GitHub is reachable, cloning from GitHub...${NC}"
            _timeout 60 git clone --depth 10 --progress https://github.com/zhayujie/ZVAgent.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
            echo -e "${YELLOW}鈿狅笍  GitHub clone failed or timed out, switching to Gitee mirror...${NC}"
            _timeout 30 git clone --depth 10 --progress https://gitee.com/zhayujie/ZVAgent.git && clone_ok=true
        fi
        if [ "$clone_ok" = false ]; then
            echo -e "${RED}鉂?Project clone failed. Please check network connection.${NC}"
            if git config --global http.proxy &> /dev/null || git config --global https.proxy &> /dev/null || [ -n "$http_proxy" ] || [ -n "$https_proxy" ] || [ -n "$HTTP_PROXY" ] || [ -n "$HTTPS_PROXY" ]; then
                echo -e "${YELLOW}馃挕 Detected proxy settings. If proxy is misconfigured, try removing it with:${NC}"
    ...
    cd ZVAgent || { echo -e "${RED}鉂?Failed to enter project directory.${NC}"; exit 1; }
    export BASE_DIR=$(pwd)
    echo -e "${GREEN}鉁?Project cloned successfully: $BASE_DIR${NC}"
    
    # Add execute permission to management script
    if [ -f "${BASE_DIR}/run.sh" ]; then
        chmod +x "${BASE_DIR}/run.sh" 2>/dev/null || true
        echo -e "${GREEN}鉁?Execute permission added to run.sh${NC}"
    fi
...
install_dependencies() {
    echo -e "${GREEN}馃摝 Installing dependencies...${NC}"
    local PIP_MIRROR=""
    ...
    [ $? -ne 0 ] && echo -e "${YELLOW}鈿狅笍  Some tools failed to upgrade, but continuing...${NC}"
    set -e
    ...
    if [ $exit_code -eq 0 ]; then
        echo -e "${GREEN}鉁?Dependencies installed successfully.${NC}"
    elif grep -qE "distutils installed project|uninstall-no-record-file|installed by debian" /tmp/pip_install.log; then
        echo -e "${YELLOW}鈿狅笍  Detected system package conflict, retrying with workaround...${NC}"
        ...
        $PYTHON_CMD -m pip install -r requirements.txt $IGNORE_PACKAGES $PIP_EXTRA_ARGS $PIP_MIRROR \
            && echo -e "${GREEN}鉁?Dependencies installed successfully (workaround applied).${NC}" \
            || echo -e "${YELLOW}鈿狅笍  Some dependencies may have issues, but continuing...${NC}"
        set -e
    elif grep -q "externally-managed-environment" /tmp/pip_install.log; then
        echo -e "${YELLOW}鈿狅笍  Detected externally-managed environment, retrying with --break-system-packages...${NC}"
        set +e
        $PYTHON_CMD -m pip install -r requirements.txt --break-system-packages $PIP_MIRROR \
            && echo -e "${GREEN}鉁?Dependencies installed successfully (system packages override applied).${NC}" \
            || echo -e "${YELLOW}鈿狅笍  Some dependencies may have issues, but continuing...${NC}"
        set -e
    else
        echo -e "${YELLOW}鈿狅笍  Installation had errors, but continuing...${NC}"
    fi

    rm -f /tmp/pip_install.log

    # Register `zv` CLI command via editable install
    echo -e "${YELLOW}Registering zv CLI...${NC}"
    set +e
    $PYTHON_CMD -m pip install -e . $PIP_EXTRA_ARGS $PIP_MIRROR > /dev/null 2>&1
    if command -v zv &> /dev/null; then
        echo -e "${GREEN}鉁?zv CLI registered.${NC}"
    else
        echo -e "${YELLOW}鈿狅笍  zv CLI not in PATH, you can still use: $PYTHON_CMD -m cli.cli${NC}"
    fi
    set -e
}
...
select_channel() {
    ...
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${YELLOW}1) Weixin (寰俊)${NC}"
    echo -e "${YELLOW}2) Feishu (椋炰功)${NC}"
    echo -e "${YELLOW}3) DingTalk (閽夐拤)${NC}"
    echo -e "${YELLOW}4) WeCom Bot (浼佸井鏅鸿兘鏈哄櫒浜?${NC}"
    echo -e "${YELLOW}5) QQ (QQ 鏈哄櫒浜?${NC}"
    echo -e "${YELLOW}6) WeCom App (浼佸井鑷缓搴旂敤)${NC}"
    echo -e "${YELLOW}7) Web (缃戦〉)${NC}"
    echo ""
...
create_config_file() {
    echo -e "${GREEN}馃摑 Generating config.json...${NC}"
...
"
    ...
    echo -e "${GREEN}鉁?Configuration file created successfully.${NC}"
}

# Start project
start_project() {
    echo ""
    echo -e "${GREEN}${EMOJI_ROCKET} Starting ZVAgent...${NC}"
    sleep 1
    ...
    if $USE_ZV; then
        ...
        if [[ "$OS_TYPE" == "Linux" ]]; then
            ...
            echo -e "${GREEN}${EMOJI_ZV} ZVAgent started on Linux (using $PYTHON_CMD)${NC}"
        elif [[ "$OS_TYPE" == "Darwin" ]]; then
            nohup $PYTHON_CMD "${BASE_DIR}/app.py" > "${BASE_DIR}/nohup.out" 2>&1 &
            echo -e "${GREEN}${EMOJI_ZV} ZVAgent started on macOS (using $PYTHON_CMD)${NC}"
        else
            echo -e "${RED}鉂?Unsupported OS: ${OS_TYPE}${NC}"
            exit 1
        fi
    fi
    ...
    echo ""
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${GREEN}${EMOJI_CHECK} ZVAgent is now running in background!${NC}"
    ...
    if $USE_ZV; then
        echo -e "  ${GREEN}zv stop${NC}       Stop the service"
        echo -e "  ${GREEN}zv restart${NC}    Restart the service"
        echo -e "  ${GREEN}zv status${NC}     Check status"
        echo -e "  ${GREEN}zv logs${NC}       View logs"
        echo -e "  ${GREEN}zv update${NC}     Update and restart"
        echo -e "  ${GREEN}zv install-browser${NC}  Install browser tool"
    else
...
show_usage() {
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_ZV} ZVAgent Management Script${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
...
# Check if zv CLI is available
has_zv() {
    command -v zv &> /dev/null
}
...
cmd_start() {
    ...
    if has_zv; then
        ...
        if is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} ZVAgent is already running (PID: $(get_pid))${NC}"
...
cmd_stop() {
    if has_zv; then
        ...
        echo -e "${GREEN}${EMOJI_STOP} Stopping ZVAgent...${NC}"

        if ! is_running; then
            echo -e "${YELLOW}${EMOJI_WARN} ZVAgent is not running${NC}"
            return
        fi

        pid=$(get_pid)
        if [ -z "$pid" ] || ! echo "$pid" | grep -qE '^[0-9]+$'; then
            echo -e "${RED}鉂?Failed to get valid PID (got: ${pid})${NC}"
            return 1
        fi
        ...
        if ps -p ${pid} > /dev/null 2>&1; then
            echo -e "${YELLOW}鈿狅笍  Process not stopped, forcing termination...${NC}"
            kill -9 ${pid}
        fi

        echo -e "${GREEN}${EMOJI_CHECK} ZVAgent stopped${NC}"
    fi
}
...
cmd_status() {
    if has_zv; then
        ...
        echo -e "${CYAN}${BOLD}=========================================${NC}"
        echo -e "${CYAN}${BOLD}   ${EMOJI_ZV} ZVAgent Status${NC}"
        echo -e "${CYAN}${BOLD}=========================================${NC}"

        if is_running; then
            pid=$(get_pid)
            echo -e "${GREEN}Status:${NC} 鉁?Running"
            echo -e "${GREEN}PID:${NC}    ${pid}"
            if [ -f "${BASE_DIR}/nohup.out" ]; then
                echo -e "${GREEN}Logs:${NC}   ${BASE_DIR}/nohup.out"
            fi
        else
            echo -e "${YELLOW}Status:${NC} 猸?Stopped"
        fi
...
cmd_logs() {
    if has_zv; then
        ...
        if [ -f "${BASE_DIR}/nohup.out" ]; then
            ...
            echo -e "${RED}鉂?Log file not found: ${BASE_DIR}/nohup.out${NC}"
        fi
    fi
}
...
# Reconfigure
cmd_config() {
    echo -e "${YELLOW}${EMOJI_WRENCH} Reconfiguring ZVAgent...${NC}"
    
    if [ -f "${BASE_DIR}/config.json" ]; then
        backup_file="${BASE_DIR}/config.json.backup.$(date +%s)"
        cp "${BASE_DIR}/config.json" "${backup_file}"
        echo -e "${GREEN}鉁?Backed up config to: ${backup_file}${NC}"
    fi
...
# Update project
cmd_update() {
    echo -e "${GREEN}${EMOJI_WRENCH} Updating ZVAgent...${NC}"
    cd "${BASE_DIR}"
    
    # Pull latest code first (service still running)
    local pull_ok=false
    if [ -d .git ]; then
        echo -e "${GREEN}馃攧 Pulling latest code...${NC}"
        if git pull; then
            pull_ok=true
        else
            echo -e "${YELLOW}鈿狅笍  git pull failed, trying Gitee mirror...${NC}"
            git remote set-url origin https://gitee.com/zhayujie/ZVAgent.git
            if git pull; then
                pull_ok=true
            else
                echo -e "${RED}鉂?Failed to pull code. Update aborted.${NC}"
                exit 1
            fi
        fi
    else
        echo -e "${YELLOW}鈿狅笍  Not a git repository, skipping code update${NC}"
    fi
...
install_mode() {
    clear
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo -e "${CYAN}${BOLD}   ${EMOJI_ZV} ZVAgent Installation${NC}"
    echo -e "${CYAN}${BOLD}=========================================${NC}"
    echo ""
    sleep 1

    if [ "$IS_PROJECT_DIR" = true ]; then
        echo -e "${GREEN}鉁?Detected existing project directory.${NC}"
        
        if [ -f "${BASE_DIR}/config.json" ]; then
            echo -e "${GREEN}鉁?Project already configured${NC}"
            echo ""
            show_usage
            return
        fi
        
        echo -e "${YELLOW}馃摑 No config.json found. Let's configure your project!${NC}"
        echo ""
    ...
    echo ""
    read -p "Start ZVAgent now? [Y/n]: " start_now
    if [[ ! $start_now == [Nn]* ]]; then
        start_project
    else
        echo -e "${GREEN}鉁?Installation complete!${NC}"
        echo ""
