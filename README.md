...
**ZVAgent** 是基于大模型的超级 AI 助理，能够主动思考和任务规划、操作计算机和外部资源、创造和执行 Skills、拥有长期记忆和知识库并不断成长，比 OpenClaw 更轻量和便捷。ZVAgent 支持灵活切换多种模型，能处理文本、语音、图片、文件等多模态消息，可接入微信、飞书、钉钉、企微智能机器人、QQ、企微自建应用、微信公众号、网页中使用，7*24小时运行于你的个人电脑或服务器中。

<p align="center">
  <a href="https://zvagent.ai/">🌐 官网</a> &nbsp;·&nbsp;
  <a href="https://docs.zvagent.ai/">📖 文档中心</a> &nbsp;·&nbsp;
  <a href="https://docs.zvagent.ai/guide/quick-start">🚀 快速开始</a> &nbsp;·&nbsp;
  <a href="https://skills.zvagent.ai/">🧩 技能广场</a> &nbsp;·&nbsp;
  <a href="https://link-ai.tech/zvagent/create">☁️ 在线体验</a>
</p>
...
-  ✅  **技能系统：** Skills 安装和运行的引擎，支持从 [Skill Hub](https://skills.zvagent.ai/)、GitHub 等一键安装技能，或通过对话创造 Skills
...
3. ZVAgent 项目专注于开源技术开发，不会参与、授权或发行任何加密货币。

## 演示

- 使用说明( Agent 模式)：[ZVAgent 介绍](https://docs.zvagent.ai/intro/features)

- 免部署在线体验：[ZVAgent](https://link-ai.tech/zvagent/create)

- DEMO 视频(对话模式)：https://cdn.link-ai.tech/doc/zvagent_demo.mp4
...
> [LinkAI](https://link-ai.tech/) 是面向企业和个人的一站式 AI 智能体平台，聚合多模态大模型、知识库、技能、工作流等能力，支持一键接入主流平台并管理，支持 SaaS、私有化部署等多种模式，可免部署在线运行[ZVAgent 助理](https://link-ai.tech/zvagent/create)。
>
...
>**2026.05.06：** [2.0.8版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.8)，飞书渠道全面升级（语音、流式输出和Markdown、一键扫码接入）、新模型支持（DeepSeek V4、百度千帆）、定时任务工具增强等

>**2026.04.22：** [2.0.7版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.7)，图像生成内置技能（GPT Image 2、Nano Banana 等）、新模型支持（Kimi K2.6、Claude Opus 4.7、GLM 5.1）、知识库和记忆增强、Web 控制台优化

>**2026.04.14：** [2.0.6版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.6)，知识库系统、梦境记忆模块、上下文智能压缩、Web 控制台多会话及多项优化。

>**2026.04.01：** [2.0.5版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.5)，ZV CLI 命令系统、Skill Hub 开源、浏览器工具、企微扫码创建、多项优化和修复。

>**2026.03.22：** [2.0.4版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.4)，新增个人微信通道（微信扫码即用）、新增 MiniMax-M2.7 和 GLM-5-Turbo 模型、run.sh 脚本重构、日文文档及多项修复。

>**2026.03.18：** [2.0.3版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.3)，新增企微智能机器人和 QQ 通道、支持 Coding Plan、新增多个模型、Web 端文件处理、记忆系统升级。

>**2026.02.27：** [2.0.2版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.2)，Web 控制台全面升级（流式对话、模型/技能/记忆/通道/定时任务/日志管理）、支持多通道同时运行、会话持久化存储、新增多个模型。

>**2026.02.13：** [2.0.1版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.1)，内置 Web Search 工具、智能上下文裁剪策略、运行时信息动态更新、Windows 兼容性适配，修复定时任务记忆丢失、飞书连接等多项问题。

>**2026.02.03：** [2.0.0版本](https://github.com/zhayujie/ZVAgent/releases/tag/2.0.0)，正式升级为超级 Agent 助理，支持多轮任务决策、具备长期记忆、实现多种系统工具、支持 Skills 框架，新增多种模型并优化了接入渠道。

更多更新历史请查看: [更新日志](https://docs.zvagent.ai/releases)
...
脚本使用说明：[一键运行脚本](https://docs.zvagent.ai/guide/quick-start)。安装后可使用 `zv start`、`zv stop` 等 [CLI 命令](https://docs.zvagent.ai/cli/index) 管理服务。
...
若遇到网络问题可使用国内仓库地址：https://gitee.com/zhayujie/ZVAgent
...
安装后可使用 `zv` 命令管理服务（启动、停止、更新等）和技能，详见 [命令文档](https://docs.zvagent.ai/cli/index)。
...
该命令会自动安装 `playwright` 和 Chromium 浏览器，国内网络自动使用镜像加速。详见 [浏览器工具文档](https://docs.zvagent.ai/tools/browser)。
...
# config.json 文件内容示例
{
  ...
  "agent_workspace": "~/zvagent",                                 # Agent 的工作空间路径，用于存储 memory、skills、系统设定等
...
+ `model`: 模型名称，Agent 模式下推荐使用 `deepseek-v4-flash`、`MiniMax-M2.7`、`glm-5.1`、`kimi-k2.6`、`qwen3.6-plus`、`claude-sonnet-4-6`、`gemini-3.1-pro-preview`，全部模型名称参考[common/const.py](https://github.com/zhayujie/ZVAgent/blob/master/common/const.py)文件
...
注：全部配置项说明可在 [`config.py`](https://github.com/zhayujie/ZVAgent/blob/master/config.py) 文件中查看。
...
推荐通过 Web 控制台在线管理模型配置，无需手动编辑文件，详见 [模型文档](https://docs.zvagent.ai/models)。以下是手动修改 `config.json` 配置模型的说明：
...
目前支持阿里云、MiniMax、智谱 GLM、Kimi、火山引擎等厂商，各厂商详细配置请参考 [Coding Plan 文档](https://docs.zvagent.ai/models/coding-plan)。
</details>


## 通道说明

推荐通过 Web 控制台在线管理通道配置，无需手动编辑文件，详见 [通道文档](https://docs.zvagent.ai/channels/weixin)。以下为手动修改 `config.json` 配置通道的说明：
...
启动后终端会显示二维码，使用微信扫码授权即可，也可以在 Web 控制台的「通道」页面中扫码接入。登录凭证会自动保存至 `~/.weixin_zvagent_credentials.json`，下次启动无需重新扫码，如需重新登录删除该文件后重启即可。

详细步骤和参数说明参考 [微信接入](https://docs.zvagent.ai/channels/weixin)
...
飞书使用 WebSocket 长连接模式，无需公网 IP。详细步骤参考 [飞书接入](https://docs.zvagent.ai/channels/feishu)。

**方式一：扫码一键创建（推荐）**

启动 ZVAgent 后打开 Web 控制台，**通道** → **接入通道** → 选择 **飞书** → 扫码创建。也支持 CLI 启动时在终端打印二维码。
...
```
详细步骤和参数说明参考 [钉钉接入](https://docs.zvagent.ai/channels/dingtalk)
</details>

<details>
<summary>5. WeCom Bot - 企微智能机器人</summary>

企微智能机器人使用 WebSocket 长连接模式，无需公网 IP 和域名。详细步骤参考 [企微智能机器人接入](https://docs.zvagent.ai/channels/wecom-bot)。

**方式一：扫码一键创建（推荐）**

启动 ZVAgent 后打开 Web 控制台，**通道** → **接入通道** → 选择 **企微智能机器人** → 使用企业微信扫码创建。
...
```
详细步骤和参数说明参考 [QQ 机器人接入](https://docs.zvagent.ai/channels/qq)
...
```
详细步骤和参数说明参考 [企微自建应用接入](https://docs.zvagent.ai/channels/wecom)
...
详细步骤和参数说明参考 [微信公众号接入](https://docs.zvagent.ai/channels/wechatmp)
...
- [ZV Skill Hub](https://github.com/zhayujie/zv-skill-hub)：开源的 AI Agent 技能广场，浏览、搜索、安装和发布技能，支持 ZVAgent、OpenClaw、Claude Code 等多种 Agent。
...
欢迎接入更多应用通道，参考 [飞书通道](https://github.com/zhayujie/ZVAgent/blob/master/channel/feishu/feishu_channel.py) 新增自定义通道，实现接收和发送消息逻辑即可完成接入。同时欢迎贡献新的 Skills，向 [Skill Hub](https://skills.zvagent.ai/submit) 提交技能。

# ✉ 联系

欢迎提交PR、Issues进行反馈，以及通过 🌟Star 支持并关注项目更新。项目运行遇到问题可以查看 [常见问题列表](https://github.com/zhayujie/ZVAgent/wiki/FAQs) ，以及前往 [Issues](https://github.com/zhayujie/ZVAgent/issues) 中搜索。个人开发者可加入开源交流群参与更多讨论，企业用户可联系[产品客服](https://cdn.link-ai.tech/portal/linkai-customer-service.png)咨询。
...
于 2026.04.13 正式更名为 **ZVAgent**。GitHub 已自动设置重定向，原有链接仍可正常访问。

如需更新本地仓库的远程地址（可选）：
```bash
git remote set-url origin https://github.com/zhayujie/ZVAgent.git
```
