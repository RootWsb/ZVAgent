...
Use this skill only when the task needs external web data that cannot be handled by ordinary web fetch/search.

- Before running a paid or large scrape, estimate cost and result volume, then ask for confirmation.
- Default to quick-answer mode with small limits for research, AI-tech, economics, and crypto monitoring.
- Do not scrape login-only, private, personal, or sensitive data unless the user clearly owns or is authorized to access it.
- Do not bypass paywalls, authentication, robots restrictions, or platform anti-abuse controls.
- Store outputs under the relevant workspace knowledge folder only when the user asks to archive results.
