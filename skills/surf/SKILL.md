...
## ZVAgent Local Safety Policy

This project uses Surf as a read-only crypto intelligence source by default.

- Do not execute trades, swaps, transfers, bridge operations, approvals, signing flows, or any command that can change funds, positions, permissions, or account state.
- Do not request, display, store, or infer private keys, seed phrases, exchange API secrets, wallet auth tokens, or signing payloads.
- Do not append routing rules to `AGENTS.md`, `CLAUDE.md`, or any project file automatically. If routing rules are useful, propose the change instead of writing it.
- Do not run `git add`, `git commit`, or other repository mutation commands from this skill.
- Prefer market, news, project, token, on-chain read, wallet-read, social, fund, and prediction-market data queries.
- Financial output must include risk framing and must not be phrased as deterministic buy/sell advice.
