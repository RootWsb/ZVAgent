...
2. If many pages may match, use `knowledge-compression-indexer` to build a small context pack before reading source pages
3. Read specific pages with the `read` tool
4. Supplement with `memory_search` if needed
...
- **Optimize when growing**: when a category has many pages, use `knowledge-compression-indexer` to update `knowledge/_meta/knowledge_map.md`, create a compaction plan, and summarize repeated ideas into domain summaries.

## Domain Layout

For this assistant's first vertical version, prefer these top-level categories when they fit:

- `knowledge/research/` — papers, literature maps, reproduction notes, experiment logs, and paper-code alignment.
- `knowledge/crypto/` — market briefs, asset notes, token events, DeFi/protocol analysis, regulation, and durable narratives.
- `knowledge/economics/` — macro briefs, data releases, central bank policy, fiscal policy, cycles, and country/sector themes.
- `knowledge/ai-tech/` — AI technical digests, model updates, engineering posts, open-source projects, papers, and architecture notes.

- `knowledge/_meta/` - lightweight maps, compaction plans, and domain summaries generated by `knowledge-compression-indexer`.

If a page naturally belongs to a more specific subfolder, create it under the relevant domain, then update `knowledge/index.md` with a one-line summary.
