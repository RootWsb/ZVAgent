...
Default this skill to read-only Dataset Viewer API usage.

- Do not create repositories, upload files, sync traces, or publish datasets unless the user explicitly asks for that write action.
- Treat local agent traces and session files as sensitive. Do not upload them by default.
- Prefer public dataset metadata, split/schema inspection, row previews, search, filter, parquet discovery, and statistics for AI technical analysis.
