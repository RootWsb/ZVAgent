---
title: Architecture
description: ZVAgent 2.0 system architecture and core design
---

ZVAgent 2.0 has evolved from a simple chatbot into a super intelligent assistant with Agent architecture, featuring autonomous thinking, task planning, long-term memory, and skill extensibility.

## System Architecture

ZVAgent's architecture consists of the following core modules:

<img src="https://cdn.link-ai.tech/doc/zvagent-arch-en.jpg.jpg" alt="ZVAgent Architecture" />
...
| **CLI** | Command-line system providing terminal commands (`zv`) and chat commands (`/`) for process management, skill installation, configuration, knowledge base management, and more |

## Agent Mode Workflow

When Agent mode is enabled, ZVAgent runs as an autonomous agent with the following workflow:
...
The Agent workspace is located at `~/zvagent` by default and stores system prompts, memory files, and skill files:
...
Secret keys are stored separately in `~/.zvagent` directory for security:
...
| `agent_workspace` | Workspace path | `~/zvagent` |
