---
title: Features
description: ZVAgent long-term memory, task planning, skills system, CLI commands, and browser tool in detail
---
...
On first launch, the Agent proactively asks the user for key information and records it in the workspace (default `~/zvagent`) — including agent settings, user identity, and memory files.
...
The Agent automatically organizes valuable information from conversations into knowledge pages, maintaining cross-references and indexes. The Web console provides document browsing and knowledge graph visualization. Knowledge is stored in `~/zvagent/knowledge/` within the workspace.
...
The built-in `browser` tool allows the Agent to control a Chromium browser to visit web pages, fill forms, click elements, and take screenshots, with support for dynamic JS-rendered pages. Run `zv install-browser` to install with one command, automatically adapting to server (headless) and desktop environments:
...
- **[Skill Hub](https://skills.zvagent.ai/):** An open skill marketplace featuring official, community, and third-party skills. Install with one command.
...
- **Custom skills:** Created by users through conversation, stored in the workspace (`~/zvagent/skills/`), capable of implementing any complex business process or third-party integration.

Install skills: `/skill install <name>` or `zv skill install <name>`, supporting Skill Hub, GitHub, ClawHub, URL, and more.
...
Visit [skills.zvagent.ai](https://skills.zvagent.ai/) to browse all available skills, or use commands in conversation:
...
ZVAgent provides two command interaction methods, covering service management, skill installation, configuration, and more:

- **Terminal CLI:** Run `zv <command>` in the system terminal, supporting `start`, `stop`, `restart`, `update`, `status`, `logs`, `skill`, etc.
...
zv update             # Update and restart
zv skill install pptx # Install a skill
zv install-browser    # Install browser tool
```

See [Command Overview](https://docs.zvagent.ai/en/cli) for details.
