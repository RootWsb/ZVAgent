...
Skill management commands are used to install, query, and manage ZVAgent skills. Use `/skill <subcommand>` in chat or `zv skill <subcommand>` in the terminal.
...
```bash Terminal
zv skill search pptx
```
</CodeGroup>

## install

Install skills with a single `install` command from ZV Skill Hub, GitHub, ClawHub, or any URL (zip archives, SKILL.md links) — no manual download or configuration required.
...
```bash Terminal
# Install all skills in a repo (auto-discovers subdirectories with SKILL.md)
zv skill install larksuite/cli

# Specify a subdirectory to install a single skill
zv skill install https://github.com/larksuite/cli/tree/main/skills/lark-im

# Use # to specify a subdirectory
zv skill install larksuite/cli#skills/lark-minutes
```
</CodeGroup>
...
```bash Terminal
zv skill install clawhub:baidu-search
```
</CodeGroup>
...
```bash Terminal
# Install from a zip archive (single or batch)
zv skill install https://cdn.link-ai.tech/skills/pptx.zip
...
```bash Terminal
zv skill enable pptx
zv skill disable pptx
```
</CodeGroup>
...
| `zvhub` | Installed from ZVAgent Skill Hub |
