...
description: Deploy ZVAgent manually (source code / Docker)
---
...
<Tip>
  For network issues, use the mirror: https://gitee.com/zhayujie/ZVAgent
</Tip>
...
<Note>
  This step is recommended. After installation you can use `zv start`, `zv stop`, `zv update` to manage the service, and `zv skill` to manage skills. Without the CLI, you can use `./run.sh` or `python3 app.py` to run.
</Note>
...
| `agent_workspace` | Agent workspace path | `~/zvagent` |
| `agent_max_context_tokens` | Max context tokens | `40000` |
| `agent_max_context_turns` | Max context turns | `30` |
| `agent_max_steps` | Max decision steps per task | `15` |

<Tip>
  Full configuration options are in the project [`config.py`](https://github.com/zhayujie/ZVAgent/blob/master/config.py).
</Tip>
