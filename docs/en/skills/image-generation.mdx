...
Send an API key in the chat and the Agent will save it to `~/zvagent/.env` using the `env_config` tool — **no restart needed**. For example:
...
In the terminal: `zv skill disable image-generation` / `zv skill enable image-generation`.
