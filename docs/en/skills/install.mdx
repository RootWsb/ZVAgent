...
ZVAgent supports installing skills from **ZV Skill Hub, GitHub, ClawHub**, and any URL with a unified `install` command. Use `/skill install` in chat or `zv skill install` in the terminal.
...
<Tip>
  All commands above work in the terminal by replacing `/skill` with `zv skill`. See [Skill Management Commands](/en/cli/skill) for full documentation.
</Tip>
