...
| `metadata.zvagent.requires.bins` | System CLI tools that must be installed |
| `metadata.zvagent.requires.env` | Required environment variables (all must be present) |
| `metadata.zvagent.requires.anyEnv` | Multiple API keys — at least one must be set |
| `metadata.zvagent.requires.anyBins` | Multiple tools — at least one must be installed |
| `metadata.zvagent.always` | Set to `true` to always load, skipping dependency checks |
| `metadata.zvagent.emoji` | Display emoji (optional) |
| `metadata.zvagent.os` | OS restriction, e.g. `["darwin", "linux"]` |
