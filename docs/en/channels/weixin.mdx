---
title: WeChat
description: Connect ZVAgent to personal WeChat
---

> Connect ZVAgent to your personal WeChat. Simply scan a QR code to log in — no public IP required. Supports text, image, voice, file, and video messages.
...
Login credentials are automatically saved to `~/.weixin_zvagent_credentials.json`. To force a re-login, delete this file and restart.
...
After successful login, credentials are saved to `~/.weixin_zvagent_credentials.json`. Subsequent startups will reuse the saved credentials without requiring a new scan.
