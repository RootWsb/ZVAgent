---
title: 一键安装
description: 使用脚本一键安装和管理 ZVAgent
---
...
| `zv logs` | 查看实时日志 |
| `zv update` | 更新代码并重启 |
| `zv install-browser` | 安装浏览器工具依赖 |

更多命令和用法参考 [命令文档](/cli/index)。

<Note>
  如果 `zv` 命令不可用，也可以使用 `./run.sh <命令>`（Linux/macOS）或 `.\scripts\run.ps1 <命令>`（Windows）作为替代，功能等效。
</Note>
