---
title: 手动安装
description: 手动部署 ZVAgent（源码 / Docker）
---
...
<Note>
  此步骤为推荐操作。安装后可以使用 `zv start`、`zv stop`、`zv update` 等命令管理服务，也可以使用 `zv skill` 管理技能。如果不安装 CLI，可以使用 `./run.sh` 或 `python3 app.py` 运行。
</Note>
...
| `agent_workspace` | - | Agent 工作空间路径 | `~/zvagent` |
...
<Tip>
  全部配置项可在项目 [`config.py`](https://github.com/zhayujie/ZVAgent/blob/master/config.py) 文件中查看。Docker 部署时，配置项名称需转为大写环境变量格式。
</Tip>
