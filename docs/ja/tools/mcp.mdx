...
ZVAgent は [Model Context Protocol (MCP)](https://modelcontextprotocol.io) をサポートしており、コミュニティで提供されている数万種類の MCP Tool を Agent から直接呼び出せます。`mcp.json` を一度設定すれば、組み込みToolとまったく同じ形で LLM に公開され、自動的に選択・呼び出されます。

## 設定ファイル

ZVAgent は `~/zvagent/mcp.json` を読み込みます。ファイルが存在しない場合は MCP Tool は読み込まれず、エラーにもなりません。

Docker デプロイの場合、公式の `docker-compose.yml` はホスト側の `./zvagent` をコンテナ内の `/home/agent/zvagent`（コンテナユーザーの `~/zvagent`）にマウント済みです。ホスト側の `./zvagent/` に `mcp.json` を置くだけで反映されます。
...
ZVAgent には `read` / `write` / `edit` Tool が組み込まれているため、**MCP の設定をそのまま Agent に渡して、ファイルに書き込んでもらえます**：

例：

```markdown
この MCP を ~/zvagent/mcp.json に追加してください：
...
- **ホットリロード**：ユーザーまたは Agent が `mcp.json` を変更すると、メッセージ処理完了時に変更された server のみが自動でリロードされます。ZVAgent の再起動は不要です
...
| 症状 | 確認ポイント |
| --- | --- |
| 起動後に MCP Tool が一つもない | `~/zvagent/mcp.json` が存在し、JSON が正しいか確認 |
...
| Docker デプロイ | ホストの `./zvagent` がコンテナ内の `/home/agent/zvagent` にマウントされていることを確認し、ホスト側の `./zvagent/` に `mcp.json` を配置。または Agent に直接インストールを依頼 |
...
MCP 標準プロトコル（stdio / SSE）に準拠していれば、コードを一切変更せずに ZVAgent に統合できます。
