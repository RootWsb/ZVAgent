...
スキル管理コマンドは ZVAgent のスキルのインストール、検索、管理に使用します。チャットでは `/skill <サブコマンド>`、ターミナルでは `zv skill <サブコマンド>` を使用します。
...
統一された `install` コマンドで、ZV Skill Hub、GitHub、ClawHub、任意の URL（zip アーカイブ、SKILL.md リンク）からスキルをワンクリックでインストールできます。手動ダウンロードや設定は不要です。
...
| --- | --- |
| `builtin` | プロジェクト内蔵スキル |
| `zvhub` | ZVAgent Skill Hub からインストール |
