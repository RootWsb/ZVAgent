...
チャットで API Key を送信すると、Agent が `env_config` ツールで `~/zvagent/.env` に保存します。**再起動不要**でただちに反映されます。例：
...
ターミナルでは `zv skill disable image-generation` / `zv skill enable image-generation`。
