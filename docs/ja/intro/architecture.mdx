---
title: アーキテクチャ
description: ZVAgent 2.0 のシステムアーキテクチャとコア設計
---

ZVAgent 2.0 は、シンプルなチャットボットから、自律的な思考、タスク計画、長期記憶、Skill の拡張性を備えた Agent アーキテクチャのスーパーインテリジェントアシスタントへと進化しました。

## システムアーキテクチャ

ZVAgent のアーキテクチャは以下のコアモジュールで構成されています：
...
| **CLI** | コマンドラインシステム。ターミナルコマンド（`zv`）とチャットコマンド（`/`）で、プロセス管理、Skill インストール、設定変更、ナレッジベース管理などをサポート |

## Agent モードのワークフロー

Agent モードが有効な場合、ZVAgent は以下のワークフローで自律的な Agent として動作します：
...
Agent のワークスペースはデフォルトで `~/zvagent` にあり、システムプロンプト、記憶ファイル、Skill ファイルを格納しています：
...
シークレットキーはセキュリティのため `~/.zvagent` ディレクトリに別途保存されます：
...
| `agent_workspace` | ワークスペースのパス | `~/zvagent` |
