...
def _parse_channel_type(raw) -> list:
    ...
    Supports:
      - single string: "weixin"
      - comma-separated string: "weixin, qq"
      - list: ["weixin", "qq"]
    """
    ...
    def start(self, channel_names: list, first_start: bool = False):
        ...
        with self._lock:
            ...
            for name in channel_names:
                ...
                if self._primary_channel is None and not name.startswith("web"):
...
def _clear_singleton_cache(channel_name: str):
    ...
    """
    from channel.channel_factory import split_channel_spec
    base_name, instance_id = split_channel_spec(channel_name)

    if base_name in (const.WEIXIN, "wx"):
        try:
            from channel.weixin.weixin_channel import WeixinChannel
            WeixinChannel.clear_instance(instance_id)
            logger.debug(f"[ChannelManager] Cleared Weixin instance cache for {channel_name}")
        except Exception as e:
            logger.warning(f"[ChannelManager] Failed to clear Weixin instance cache: {e}")
        return
    ...
    module_path = cls_map.get(base_name)
    if not module_path:
        return
    try:
...
def _sync_builtin_skills():
    ...
    import shutil
    try:
        workspace = conf().get("agent_workspace", "runtime/workspace")
