...
class AgentBridge:
    ...
    def _is_restricted_channel(self, context: Context = None) -> bool:
        if not context:
            return False
        channel_type = str(context.get("channel_type", "") or "").strip()
        if not channel_type:
            return False
        base_channel = channel_type.split(":", 1)[0]
        restricted_channels = set(self._config_list(
            "restricted_user_channels",
            ["weixin", "wecom_bot", "wechatcom_app", "wechatmp", "wechatmp_service"],
        ))
        return channel_type in restricted_channels or base_channel in restricted_channels

    def _filter_tools_for_context(self, tools: List, context: Context = None) -> List:
        if not self._is_restricted_channel(context):
            return tools
        blocked_tools = set(self._config_list(
            "restricted_user_blocked_tools",
            ["write", "edit", "bash", "env_config", "scheduler"],
        ))
        allow_mcp = bool(conf().get("restricted_user_allow_mcp_tools", False))
        filtered = []
        removed = []
        for tool in tools:
            name = getattr(tool, "name", "")
            is_mcp_tool = tool.__class__.__name__ == "McpTool" or hasattr(tool, "server_name")
            if name in blocked_tools or (is_mcp_tool and not allow_mcp):
                removed.append(name or tool.__class__.__name__)
                continue
            filtered.append(tool)
        if removed:
            logger.info(
                f"[AgentBridge] Restricted channel {context.get('channel_type', '')}: "
                f"blocked tools {sorted(set(removed))}"
            )
        return filtered
    ...
    def agent_reply(self, query: str, context: Context = None, 
        ...
        try:
            ...
            agent._current_user_id = context_user_key(context) if context else session_id
            ...
            skill_filter = None
            if getattr(agent, "skill_manager", None):
                try:
                    skill_filter = agent.skill_manager.select_relevant_skills(query)
                    if skill_filter:
                        logger.debug(
                            f"[AgentBridge] Selected skills for this run: {skill_filter}"
                        )
                        # 记录 skill 选择决策（用于训练 SkillRouter）
                        try:
                            ...
                            logger.debug(f"[AgentBridge] Failed to record skill selection: {collect_err}")
                except Exception as e:
                    logger.warning(f"[AgentBridge] Failed to select relevant skills: {e}")
            ...
            try:
                ...
                if tools_filtered or (context and context.get("is_scheduled_task")):
            ...
            # Post-message hot-reload: detect edits to ~/zvagent/mcp.json and
            ...
            if hasattr(agent, 'stream_executor') and hasattr(agent.stream_executor, 'files_to_send'):
                ...
                if files_to_send:
                    ...
                    reply = self._create_file_reply(file_info, response, context)
                    reply.tool_events = event_handler.get_tool_events()
    ...
    def _migrate_config_to_env(self, workspace_root: str):
        ...
        read_env_file = env_file if os.path.exists(env_file) else expand_path("~/.cow/.env")
        
        # Read existing env vars (key -> value)
        existing_env_vars = {}
        if os.path.exists(read_env_file):
            try:
                with open(read_env_file, 'r', encoding='utf-8') as f:
                    for line in f:
                        line = line.strip()
    ...
    def refresh_all_skills(self) -> int:
        ...
        workspace_root = expand_path(conf().get("agent_workspace", "runtime/workspace"))
