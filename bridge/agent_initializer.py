...
class AgentInitializer:
    ...
    def _load_env_file(self):
        ...
        from config import conf
        env_file = os.path.join(expand_path(conf().get("agent_workspace", "runtime/workspace")), ".env")
        ...
        # Also load crypto_api_key from config if available
        try:
            from config import conf
            crypto_api_key = conf().get("crypto_api_key", "")
            if crypto_api_key and not os.environ.get("CRYPTO_API_KEY"):
                os.environ["CRYPTO_API_KEY"] = crypto_api_key
                logger.debug("[AgentInitializer] Loaded CRYPTO_API_KEY from config")
        except Exception:
            pass
    ...
    def _setup_memory_system(self, workspace_root: str, session_id: Optional[str] = None):
        ...
        try:
            ...
            # Add GraphQueryTool if knowledge graph is available
            if memory_manager and getattr(memory_manager, "knowledge_graph", None):
                try:
                    from agent.tools.graph.graph_query import GraphQueryTool
                    memory_tools.append(GraphQueryTool(memory_manager.knowledge_graph))
                    if session_id is None:
                        logger.info("[AgentInitializer] GraphQueryTool added")
                except Exception as e:
                    if session_id is None:
                        logger.debug(f"[AgentInitializer] GraphQueryTool not loaded: {e}")
    ...
    def _load_tools(self, workspace_root: str, memory_manager, memory_tools: List, session_id: Optional[str] = None):
        ...
        file_config = {
            "cwd": workspace_root,
            "headless": _as_bool(conf().get("browser_headless", True), True),
            "idle_timeout": conf().get("browser_idle_timeout", 120),
        }
        if memory_manager:
            file_config["memory_manager"] = memory_manager
    ...
    def _flush_all_agents(self):
        ...
        for label, agent in agents:
            try:
                ...
                if not messages:
                    continue
                user_id = None if label == "default" else label
                result = agent.memory_manager.flush_manager.create_daily_summary(messages, user_id=user_id)
                if result:
                    flushed += 1
                ...
                if dream_candidate is None:
                    dream_candidate = (agent.memory_manager.flush_manager, user_id)
            except Exception as e:
        ...
        if dream_candidate:
            try:
                flush_manager, user_id = dream_candidate
                result = flush_manager.deep_dream(user_id=user_id)
                if result:
