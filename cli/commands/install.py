"""zv install-browser - Install Playwright + Chromium for the browser tool."""
...
def run_install_browser(
    ...
    Reused by ``zv install-browser`` CLI and chat ``/install-browser``.
    ...
    stream("")
    stream("Browser tool ready! Restart ZVAgent to enable it.", "green")
    _phase(on_phase, "🎉 全部步骤结束。请重启 ZVAgent 后使用 browser 工具。")
    return 0
