"""zv start/stop/restart/status/logs - Process management commands."""
...
def _get_pid_file():
    return get_runtime_path("process", ".zvagent.pid")


def _get_legacy_pid_file():
    return os.path.join(get_project_root(), ".cow.pid")


def _get_log_file():
    return get_runtime_path("logs", "nohup.out")


def _get_python_executable() -> str:
    configured = os.environ.get("ZVAGENT_PYTHON") or load_config_json().get("python_executable", "")
    if configured:
        configured = os.path.abspath(os.path.expanduser(str(configured)))
        if os.path.exists(configured):
            return configured
        click.echo(f"Warning: configured python_executable not found: {configured}", err=True)
...
def _read_pid() -> Optional[int]:
    for pid_file in (_get_pid_file(), _get_legacy_pid_file()):
...
def start(foreground, no_logs):
    ...
    pid = _read_pid()
    if pid:
        click.echo(f"ZVAgent is already running (PID: {pid}).")
        return
    ...
    if foreground:
        click.echo("Starting ZVAgent in foreground...")
        if _IS_WIN:
        ...
        _write_pid(proc.pid)
        click.echo(click.style(f"✓ ZVAgent started (PID: {proc.pid})", fg="green"))
...
def stop():
    ...
    pid = _read_pid()
    if not pid:
        click.echo("ZVAgent is not running.")
        return

    click.echo(f"Stopping ZVAgent (PID: {pid})...")
    try:
        _kill_pid(pid)
        for _ in range(30):
            time.sleep(0.1)
    ...
    _remove_pid()
    click.echo(click.style("✓ ZVAgent stopped.", fg="green"))
...
def update(ctx):
    ...
    if _IS_WIN:
        # On Windows, `zv.exe` (this process) locks the exe file, so
        # `pip install -e .` fails with WinError 5.  Write a small .bat
        # helper that waits for zv.exe to exit, then installs & starts.
        bat = os.path.join(root, "_zvagent_update.bat")
        lines = [
            "@echo off",
            "chcp 65001 >nul",
            "echo Waiting for zv.exe to exit...",
        ...
        lines += [
            "echo Reinstalling ZVAgent CLI...",
            f'"{python}" -m pip install -e . -q',
            "echo Starting ZVAgent...",
        ...
        subprocess.Popen(
            ["cmd.exe", "/c", "start", "ZVAgent Update", "/wait", bat],
            cwd=root,
        )
        ...
        click.echo("Reinstalling ZVAgent CLI...")
        subprocess.call(
...
@click.command()
def status():
    """Show ZVAgent running status."""
    from cli import __version__
    from cli.utils import load_config_json

    pid = _read_pid()
    if pid:
        click.echo(click.style(f"● ZVAgent is running (PID: {pid})", fg="green"))
    else:
        click.echo(click.style("● ZVAgent is not running", fg="red"))
