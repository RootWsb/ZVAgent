...
Storage path: ~/zvagent/sessions/conversations.db
"""
...
_API_FILE_RE = re.compile(r"/api/file\?path=([^\]\)\s\"'<>]+)", re.IGNORECASE)


def _safe_delete_roots() -> List[str]:
    roots = []
    try:
        from common.runtime import get_runtime_dir
        roots.append(get_runtime_dir())
    except Exception:
        pass
    try:
        from common.utils import expand_path
        from config import conf
        roots.append(expand_path(conf().get("agent_workspace", "runtime/workspace")))
    except Exception:
        pass
    ...
    for root in roots:
        ...
        if abs_root not in safe_roots:
            safe_roots.append(abs_root)
    return safe_roots
...
def _normalize_candidate_path(raw_path: str) -> str:
    ...
    if path.startswith("/api/file?"):
        try:
            ...
            path = values[0] if values else path
        except Exception:
            pass
...
def _collect_local_file_refs(value: Any) -> List[str]:
    refs: List[str] = []

    def add(raw: Any):
        path = _normalize_candidate_path(str(raw or ""))
        if path:
            refs.append(path)
    ...
    def walk(node: Any):
        if isinstance(node, dict):
            ...
            if node_type in {"audio", "file_to_send"}:
                add(node.get("path") or node.get("url") or "")
            for key in ("path", "file_path"):
                if key in node and node_type in {"audio", "file_to_send", "image", "video", "document", "file"}:
                    add(node.get(key))
            ...
            text = node
            for match in _API_FILE_RE.findall(text):
                add(match)
            for match in _FILE_URL_RE.findall(text):
                add(match)
            for match in _WINDOWS_ABS_PATH_RE.findall(text):
                add(match)
            try:
    ...
    unique = []
    seen = set()
    safe_roots = _safe_delete_roots()
    for ref in refs:
        if ref in seen or not os.path.isfile(ref):
            continue
        if not any(_is_under_root(ref, root) for root in safe_roots):
            continue
        seen.add(ref)
        unique.append(ref)
    return unique
...
def _group_into_display_turns(
    ...
    # user_row: (seq, content, created_at)
    groups: List[tuple] = []
    ...
    for row in rows:
        if len(row) == 4:
            seq, role, raw_content, created_at = row
        else:
            seq = None
            role, raw_content, created_at = row
        try:
        ...
        if role == "user" and _is_visible_user_message(content):
            ...
            cur_user = (seq, content, created_at)
            cur_rest = []
            started = True
        else:
            cur_rest.append((seq, role, content, created_at))
    ...
    for user_row, rest in groups:
        # User turn
        if user_row:
            seq, content, created_at = user_row
            text = _extract_display_text(content)
            if text:
                turns.append({"role": "user", "content": text, "created_at": created_at, "_seq": seq})
        ...
        for seq, role, content, created_at in rest:
            if role == "user":
                ...
                if isinstance(content, list):
                    for block in content:
                        ...
                        if btype == "thinking":
                            ...
                            audio_path = block.get("path") or block.get("url") or ""
                            if audio_path:
                                steps.append({
                                    "type": "audio",
                                    "path": audio_path,
...
class ConversationStore:
    ...
    def append_assistant_audio(
        ...
        """Persist an assistant audio attachment for the latest visible turn."""
    ...
    def clear_session(self, session_id: str) -> Dict[str, Any]:
        ...
        Local files referenced by the session are removed when they live under
        runtime or the configured agent workspace.
        """
        ...
        with self._lock:
            ...
            try:
                ...
                raw_contents = [row[0] for row in rows]
                with conn:
                    conn.execute(
        ...
        for raw_content in raw_contents:
            try:
                ...
                parsed = raw_content
            file_refs.extend(_collect_local_file_refs(parsed))
        ...
        return {
            "deleted_messages": len(raw_contents),
            "deleted_files": deleted_files,
            "skipped_files": skipped_files,
        }

    def delete_visible_turn(self, session_id: str, user_seq: int) -> Dict[str, Any]:
        """
        Delete one visible user turn and all following internal/assistant rows
        until the next visible user message. Local generated/downloaded files
        referenced by those rows are also removed when they live under runtime
        or the configured agent workspace.
        """
            ...
            try:
                ...
                for seq, role, raw_content in rows:
                    try:
                        ...
                        parsed = raw_content
                    is_visible_user = role == "user" and _is_visible_user_message(parsed)
                    if seq == user_seq:
                        if not is_visible_user:
                            raise ValueError("message is not a visible user turn")
                        found_start = True
                    elif found_start and is_visible_user:
                        break
                    if found_start:
                        seqs_to_delete.append(seq)
                        raw_contents.append(raw_content)

                if not found_start or not seqs_to_delete:
                ...
                with conn:
                    ...
                    conn.execute(
                        ...
                        WHERE session_id = ?
                        """,
                        (session_id, int(time.time()), session_id),
                    )
        ...
        return {
            "deleted_messages": len(seqs_to_delete),
            "deleted_files": deleted_files,
            "skipped_files": skipped_files,
        }
    ...
    def load_history_page(
        ...
        # Keep seqs in display rows so the UI can delete a whole visible turn.
        plain_rows = [(seq, role, content, created_at) for seq, role, content, created_at in rows]
...
def get_conversation_store() -> ConversationStore:
    ...
    SQLite file: ~/zvagent/memory/long-term/index.db
    ...
    with _store_lock:
        ...
        try:
            ...
            from common.utils import expand_path
            db_path = Path(expand_path("runtime/workspace")) / "memory" / "long-term" / "index.db"
