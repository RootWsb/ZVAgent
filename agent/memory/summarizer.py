...
class MemoryFlushManager:
    ...
    def deep_dream(self, user_id: Optional[str] = None, lookback_days: int = 1, force: bool = False) -> bool:
        ...
        # Extract entities into knowledge graph from the new MEMORY.md
        self._extract_and_store_graph(new_memory, source_path="MEMORY.md")
    ...
    def _extract_and_store_graph(self, text: str, source_path: str = ""):
        """Extract entities from text and store in knowledge graph."""
        knowledge_graph = getattr(self, "knowledge_graph", None)
        entity_extractor = getattr(self, "entity_extractor", None)
        if not knowledge_graph or not entity_extractor or not text:
            return

        try:
            # Deactivate old triples from this source before re-extracting
            if source_path:
                knowledge_graph.deactivate_source_triples(source_path)

            triples = entity_extractor.extract_from_text(
                text, source="memory", source_path=source_path,
            )
            if triples:
                count = knowledge_graph.add_triples_batch(
                    triples, source="memory", source_path=source_path,
                )
                logger.info(f"[DeepDream] Extracted {count} triples into knowledge graph from {source_path}")
        except Exception as e:
            logger.warning(f"[DeepDream] Graph extraction failed: {e}")
