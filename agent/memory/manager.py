...
class MemoryManager:
    ...
    def __init__(
        ...
        if self.config.graph_enabled:
            try:
                ...
                graph_db_path = self.config.get_graph_db_path()
                self.knowledge_graph = KnowledgeGraph(db_path=graph_db_path)
                if llm_model:
                    from agent.knowledge.entity_extractor import EntityExtractor
                    self.entity_extractor = EntityExtractor(llm_model)
            except Exception as e:
                from common.log import logger
                logger.info(f"[MemoryManager] Knowledge graph not available: {e}")

        # Attach graph references to flush manager for dream distillation
        if self.knowledge_graph:
            self.flush_manager.knowledge_graph = self.knowledge_graph
        if self.entity_extractor:
            self.flush_manager.entity_extractor = self.entity_extractor
    ...
    async def search(
        ...
        # Augment with knowledge graph context
        if self.knowledge_graph:
            graph_results = self._search_graph(query, max_results)
    ...
    async def sync(self, force: bool = False):
        ...
        # Extract entities into knowledge graph from changed files
        await self._sync_graph_from_changed_files()
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        graph_stats = self.knowledge_graph.get_stats() if self.knowledge_graph else None
        return {
            'chunks': stats['chunks'],
            'files': stats['files'],
    ...
    def _search_graph(self, query: str, max_results: int) -> List[SearchResult]:
        """Search the knowledge graph and return synthetic SearchResult objects."""
        from common.log import logger
        try:
            triples = self.knowledge_graph.search_triples(query, limit=max_results)
            if not triples:
                return []
            ...
            logger.debug(f"[MemoryManager] Graph search failed: {e}")
            return []

    async def _sync_graph_from_changed_files(self):
        """Extract entities from recently changed files into the knowledge graph."""
        if not self.knowledge_graph or not self.entity_extractor:
            return
        ...
        try:
            ...
            if memory_file.exists():
                files_to_extract.append(("memory", memory_file))

            # Check recent daily memory files (last 3 days)
            memory_dir = self.config.get_memory_dir()
            if memory_dir.exists():
                for md_file in sorted(memory_dir.glob("*.md"), reverse=True)[:3]:
                    files_to_extract.append(("memory", md_file))
            ...
            if conf().get("knowledge", True):
                ...
                if knowledge_dir.exists():
                    for md_file in knowledge_dir.rglob("*.md"):
                        files_to_extract.append(("knowledge", md_file))

            for source, file_path in files_to_extract:
                try:
                    content = file_path.read_text(encoding="utf-8")
                    ...
                    # Deactivate old triples from this source, then re-extract
                    self.knowledge_graph.deactivate_source_triples(rel_path)

                    triples = self.entity_extractor.extract_from_text(
                        content, source=source, source_path=rel_path,
                    )
                    if triples:
                        self.knowledge_graph.add_triples_batch(
                            triples, source=source, source_path=rel_path,
                        )
                except Exception as e:
                    logger.debug(f"[MemoryManager] Graph extraction failed for {file_path}: {e}")

        except Exception as e:
            logger.debug(f"[MemoryManager] Graph sync failed: {e}")
