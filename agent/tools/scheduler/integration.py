...
def _execute_send_message(task: dict, agent_bridge):
    ...
    try:
        ...
        elif channel_type in ("qq", "weixin"):
            context["msg"] = None
...
def _execute_tool_call(task: dict, agent_bridge):
    ...
    try:
        ...
        is_group = action.get("is_group", False)
        channel_type = action.get("channel_type", "unknown")
        base_channel = str(channel_type or "").split(":", 1)[0]
        restricted_channels = conf().get(
            "restricted_user_channels",
            ["weixin", "wecom_bot", "wechatcom_app", "wechatmp", "wechatmp_service"],
        )
        if isinstance(restricted_channels, str):
            restricted_channels = [item.strip() for item in restricted_channels.split(",") if item.strip()]
        blocked_tools = conf().get(
            "restricted_user_blocked_tools",
            ["write", "edit", "bash", "env_config", "scheduler"],
        )
        if isinstance(blocked_tools, str):
            blocked_tools = [item.strip() for item in blocked_tools.split(",") if item.strip()]
        if (channel_type in restricted_channels or base_channel in restricted_channels) and tool_name in set(blocked_tools):
            logger.warning(
                f"[Scheduler] Blocked restricted channel task {task.get('id')}: "
                f"channel={channel_type}, tool={tool_name}"
            )
            return
