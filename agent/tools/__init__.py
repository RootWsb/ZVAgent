...
# GraphQueryTool (requires knowledge graph)
def _import_graph_tool():
    ...
    try:
        ...
        logger.debug(f"[Tools] GraphQueryTool not loaded: {e}")
        return None
    except Exception as e:
        logger.debug(f"[Tools] GraphQueryTool failed to load: {e}")
        return None

GraphQueryTool = _import_graph_tool()
